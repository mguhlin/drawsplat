...
window.DrawSplatVideoNotes=(()=>{
  ...
  let api,dialog,stream,recorder,draft=null,token=0,timer,started=0,finishing=false,poster='',activePanel,lastDraftState=false;
    ...
    try{
      ...
      recorder.onstop=async()=>{
        ...
        try{
          ...
          message(dialog.open?'Your clip is not saved yet. Watch it, then add it or try again.':'Unfinished clip kept on this open page. Choose Finish video to add or discard it.');
  ...
  function clearDraft(resetFields=false){
    draft=null;const preview=get('videoNotePreview');preview.pause();preview.removeAttribute('src');preview.load();
  ...
  function add(){
    ...
    if(activePanel!==api.getPanel().id){message('The page changed. Return to the page where you recorded this clip, or choose Try again.');return}
    ...
    api.addObject(object);clearDraft(true);close();api.status('Video note added. Play it on the board.','success');
  }
  function open(){if(!api.allowed())return api.status('Your teacher has not enabled video notes for this lesson.','danger');if(!dialog.open)dialog.showModal();message(draft?'Your clip is not saved yet. Watch it, then add it to the board.':'Record up to 30 seconds. You can review it before adding it.');refresh()}
  function init(adapter){
    ...
    dialog.innerHTML=`
      ...
      <p class="hint">The camera turns off when you stop or close. Add your clip to save it.</p>
      <details class="video-note-help"><summary>About saving video</summary><p class="hint">Maximum 30 seconds and 5 MB. Closing keeps an unfinished clip only on this open page. Add it before refreshing. Saved board files keep the video; PNG and PDF exports show a still picture.</p></details>`;
  ...
  function createObject(object,box){
    ...
    if(validSource(object.videoSrc)){
      let player=players.get(object.id);if(!player||player.source!==object.videoSrc){player?.video.pause();const video=document.createElementNS(HTML,'video');video.src=object.videoSrc;video.controls=true;video.playsInline=true;video.preload='metadata';video.addEventListener('play',()=>players.forEach(other=>{if(other.video!==video)other.video.pause()}));video.setAttribute('aria-label',(object.videoName||'Video note')+' playback');player={video,source:object.videoSrc,resume:false};players.set(object.id,player)}
