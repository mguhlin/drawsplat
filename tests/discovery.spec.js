...
test('homepage routes natural-language intent to the right tools', async ({ page }) => {
  const errors = []; page.on('console', message => { if (message.type() === 'error') errors.push(message.text()); });
  await page.goto('/');
  await expect(page.getByRole('heading', { name:'DrawSplat', exact:true })).toBeVisible();
  await page.locator('#homeSearch').fill('I need to make slides');
  await expect(page.locator('#homeSearchResults')).toBeVisible();
  await expect(page.locator('#homeSearchResults').getByRole('link', { name:/ShowSplat/ }).first()).toBeVisible();
  await page.getByRole('button', { name:'Apps' }).click();
  await expect(page.getByRole('dialog', { name:'What do you want to do?' })).toBeVisible();
  await page.getByRole('dialog').getByRole('searchbox').fill('pictograph');
  await expect(page.getByRole('dialog').getByRole('link', { name:/Picture Graph/ })).toBeVisible();
...
test('Studio searches aliases, filters functions, and saves favorites locally', async ({ page }) => {
  ...
  await page.reload();
  await expect(page.locator('#favoritesShelf')).toContainText('SplatImage Studio');
...
test('Studio accepts category deep links', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name:/Data/ })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.locator('.ds-tool-card h3').first()).toContainText(/Graph|Chart|Grid/);
});

test('major apps expose the shared app launcher', async ({ page }) => {
  for (const url of ['/app/whiteboard.html', '/solutions/pdfsplat/', '/splatworks/gridsplat/']) {
    await page.goto(url);
    const welcome = page.getByRole('button', { name:'Got it' });
    await welcome.waitFor({ state:'visible', timeout:2000 }).catch(() => {});
    ...
    await expect(page.getByRole('dialog').getByRole('link', { name:/GridSplat/ })).toBeVisible();
  }
});
