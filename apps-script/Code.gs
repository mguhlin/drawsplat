...
/* --- Compliance: Data export (Day 2.6) ------------------------------------- */

function exportStudentData_(p) {
  try {
    requireAdmin_(p);
    const studentName = clean_(p.studentName || '');
    const className = clean_(p.className || '');
    ...
    // Boards matching the student (boards are keyed by className + title containing student name)
    const boardRows = readRows_(DS_SHEETS.boards);
    boardRows.forEach(b => {
      const cn = String(b.className || '').trim().toLowerCase();
      const title = String(b.title || '').toLowerCase();
      const cnMatches = !className || cn === className.toLowerCase();
      const titleMatches = title.indexOf(studentName.toLowerCase()) !== -1;
      if (cnMatches && titleMatches && b.jsonFileId) {
        try {
          const json = readJsonFile_(b.jsonFileId);
          const safeName = cleanName_(b.boardId || b.title || 'board');
          blobs.push(Utilities.newBlob(JSON.stringify(json, null, 2), 'application/json', 'boards/' + safeName + '.json'));
          if (b.pngFileId) {
            try { blobs.push(DriveApp.getFileById(b.pngFileId).getBlob().setName('boards/' + safeName + '.png')); } catch (e) {}
          }
          manifest.boards.push({ boardId: b.boardId, title: b.title, className: b.className, updatedAt: b.updatedAt });
        } catch (err) {}
      }
    });
    // Turn-ins matching the student
    const turninRows = readRows_(DS_SHEETS.turnins);
    turninRows.forEach(t => {
      if (matches(t) && t.jsonFileId) {
        try {
          const json = readJsonFile_(t.jsonFileId);
          const safeName = cleanName_(t.turninId || t.title || 'turnin');
          blobs.push(Utilities.newBlob(JSON.stringify(json, null, 2), 'application/json', 'turnins/' + safeName + '.json'));
          if (t.pngFileId) {
            try { blobs.push(DriveApp.getFileById(t.pngFileId).getBlob().setName('turnins/' + safeName + '.png')); } catch (e) {}
          }
          manifest.turnins.push({ turninId: t.turninId, title: t.title, className: t.className, updatedAt: t.updatedAt });
        } catch (err) {}
      }
    });
    // User row (without hashed credentials)
    const userRow = findUserByStudent_(studentName, className);
    if (userRow) {
      manifest.user = {
        id: userRow.id,
        ...
        email: userRow.email,
        ageBand: userRow.ageBand,
        ageSource: userRow.ageSource,
        ageChangedBy: userRow.ageChangedBy,
        ageChangedAt: userRow.ageChangedAt,
        ageChangeReason: userRow.ageChangeReason,
    ...
    const readme = [
      'DrawSplat Student Data Export',
      'Generated: ' + manifest.exportedAt,
      'Student: ' + studentName + (className ? ' (' + className + ')' : ''),
      '',
      'Contents:',
      '  manifest.json   Summary of what is in this export.',
      '  boards/         JSON + PNG copies of every board matching this student.',
      '  turnins/        JSON + PNG copies of every turn-in submitted by this student.',
      '',
      'Items found: ' + manifest.boards.length + ' boards, ' + manifest.turnins.length + ' turn-ins, ' + (manifest.user ? '1' : '0') + ' user record.',
      '',
      'This export was generated by DrawSplat\'s Family Access Tools in response',
      'to a verified parent / guardian request, FERPA inspection request, or',
      'similar authorized data request.',
      '',
      'Companion documents:',
      '  Terms & Privacy        https://drawsplat.org/legal/terms-privacy.html',
      '  Family Access Tools    https://drawsplat.org/parents/'
    ].join('\n');
    blobs.push(Utilities.newBlob(readme, 'text/plain', 'README.txt'));
    if (!blobs.length) throw new Error('No matching data found for this student.');
    const filenameSafe = cleanName_(studentName.replace(/[^A-Za-z0-9_-]/g, '_'));
    const zipName = 'DrawSplat-Student-Export-' + filenameSafe + '-' + new Date().toISOString().slice(0, 10) + '.zip';
    const zip = Utilities.zip(blobs, zipName);
    logEvent_('DATA_EXPORT', {
      actor: actor, actorRole: 'admin',
      entityType: 'student_data', entityId: studentName + '|' + className,
      after: { boardsExported: manifest.boards.length, turninsExported: manifest.turnins.length, reason: clean_(p.reason || '') }
    });
    return {
      ok: true,
      filename: zipName,
      contentBase64: Utilities.base64Encode(zip.getBytes()),
      boardsExported: manifest.boards.length,
      turninsExported: manifest.turnins.length,
      hasUserRow: !!manifest.user
    };
  } catch (err) {
