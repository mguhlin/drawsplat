...
Individual app downloads now include the shared illustrated menu, language, and launcher runtime dependencies referenced by their entry pages. Public module pages also include their manifest and launcher registry. Tools includes ImageSplat, GraphSplat, and QRSplat; Widgets includes SplatBot Studio; Games includes Dots and Boxes. Launcher links to other apps require those apps to be installed in the same web root.
