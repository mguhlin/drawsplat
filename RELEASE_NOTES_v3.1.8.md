...
- Adds the DrawSplat Hub dashboard at `hub/index.html` plus the first school-specific instance scaffold at `hub/hubcampus/`, including classroom-teacher, campus, and district grouping support, the DrawSplat Hub banner, campus hero image, guided instance admin wizard, optional Buy Me a Coffee support on the admin page, Google OAuth admin binding pattern, and `apps-script/InstanceRegistry.gs` starter backend for one-time setup password plus Google account binding.
