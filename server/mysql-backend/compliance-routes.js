...
function attachComplianceRoutes(app, pool, options = {}) {
  ...
  const authLimiter = createRateLimiter({ scope: 'auth', windowMs: 15 * 60 * 1000, max: Number(process.env.AUTH_RATE_LIMIT_PER_15_MIN || 20) });
  const parentLimiter = createRateLimiter({ scope: 'parent', windowMs: 15 * 60 * 1000, max: Number(process.env.PARENT_RATE_LIMIT_PER_15_MIN || 30) });
  ...
  app.post(basePath + '/auth/register', authLimiter, async (req, res) => {
    try {
      ...
      const assignedRole = chooseSelfRegisteredRole(role, normalizedEmail);
      ...
      const [r] = await pool.execute(
        ...
        { email: normalizedEmail, displayName: safeString(displayName, 200), role: assignedRole, hash, salt }
      );
      await logEvent(pool, 'USER_CREATED', { actor: normalizedEmail, actorUserId: r.insertId, actorRole: assignedRole, targetType: 'user', targetId: String(r.insertId), metadata: { provider: 'email' } });
      res.json({ ok: true, userId: r.insertId });
    } catch (err) {
      res.status(err.code === 'ER_DUP_ENTRY' ? 409 : 500).json({ ok: false, error: err.code === 'ER_DUP_ENTRY' ? 'email_already_registered' : 'Server error' });
    }
  });

  app.post(basePath + '/auth/login', authLimiter, async (req, res) => {
    try {
      const { email, password } = req.body || {};
      const normalizedEmail = normalizeEmail(email);
      const [rows] = await pool.query('SELECT * FROM users WHERE email = ? AND deleted_at IS NULL LIMIT 1', [normalizedEmail]);
      const user = rows[0];
  ...
  app.post(basePath + '/parent/request', parentLimiter, async (req, res) => {
    try {
      ...
      const parentNameSafe = safeString(parentName, 200);
      const studentNameSafe = safeString(studentName, 200);
      const classNameSafe = safeString(className, 200);
      const requestTypeSafe = safeString(requestType, 80);
      if (!parentNameSafe || !isValidEmail(normalizedParentEmail) || !studentNameSafe || !requestTypeSafe) return res.status(400).json({ ok: false, error: 'Missing required fields.' });
      let status = 'pending_verification';
      let studentUserId = null;
      const [students] = await pool.query('SELECT * FROM users WHERE student_name = ? AND class_name = ? AND role = "student" LIMIT 1', [studentNameSafe, classNameSafe || '']);
      if (students[0]) {
      ...
      await pool.execute(
        ...
        { id, pn: parentNameSafe, pe: normalizedParentEmail, sid: studentUserId, sn: studentNameSafe, cn: classNameSafe || null, rt: requestTypeSafe, d: safeString(details, 4000) || null, s: status }
      );
      await logEvent(pool, 'PARENT_REQUEST_CREATED', { actor: normalizedParentEmail, actorRole: 'parent', targetType: 'parent_request', targetId: id, metadata: { requestType: requestTypeSafe, status } });
