(function () {
  ...
  function webDeckJs() {
    return [
      ...
      'function go(n){i=Math.max(0,Math.min(SLIDES.length-1,n));render();showControls()}',
      ...
      'document.addEventListener("keydown",e=>{showControls();if(["ArrowRight"," ","ArrowDown"].includes(e.key))go(i+1);if(["ArrowLeft","ArrowUp"].includes(e.key))go(i-1);if(e.key==="Home")go(0);if(e.key==="End")go(SLIDES.length-1);if(e.key.toLowerCase()==="n")openNotes();if(e.key.toLowerCase()==="f")document.documentElement.requestFullscreen?.();});',
