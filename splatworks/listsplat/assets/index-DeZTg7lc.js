...
`||o===`\r`)?(o===`\r`&&s===`
`&&(a+=1),r.push(n),r.some(e=>e.length>0)&&t.push(r),r=[],n=``):n+=o}return r.push(n),r.some(e=>e.length>0)&&t.push(r),t}function f(e,t){let i=re(t),a=(i[0]?.map((e,t)=>e.trim()||`Field ${t+1}`)??[`Field 1`]).map(e=>n(e)),o=i.slice(1).map(e=>r(a,Object.fromEntries(a.map((t,n)=>[t.id,e[n]??``]))));return{id:`table_${Date.now().toString(36)}`,name:e,fields:a,records:o.length>0?o:[r(a)]}}function p(e){let t=e==null?``:String(e);return/[",\n\r]/.test(t)?`"${t.replaceAll(`"`,`""`)}"`:t}function ie(e){return[e.fields.map(e=>p(e.name)).join(`,`),...e.records.map(t=>e.fields.map(e=>p(t.values[e.id])).join(`,`))].join(`
`)}var m=`listsplat.autosave.v1`;function h(e){localStorage.setItem(m,JSON.stringify(e))}function ae(){let e=localStorage.getItem(m);if(!e)return null;let t=JSON.parse(e);return d(t),t}function g(e,t,n){let r=new Blob([t],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=e,a.click(),URL.revokeObjectURL(i)}function oe(e,t){return e.records.map(e=>Number(e.values[t])).filter(e=>Number.isFinite(e))}function _(e){return e.fields.filter(e=>[`number`,`currency`,`percent`].includes(e.type)).map(t=>{let n=oe(e,t.id),r=n.reduce((e,t)=>e+t,0);return{fieldId:t.id,fieldName:t.name,count:n.length,sum:r,average:n.length?r/n.length:0,minimum:n.length?Math.min(...n):0,maximum:n.length?Math.max(...n):0}})}function se(e,t,n){let r=e.trim(),i=r.match(/^FIELD\(([^)]+)\)$/i);if(i){let e=i[1].trim().toLowerCase(),r=t.fields.find(t=>t.name.toLowerCase()===e);return r?String(n.values[r.id]??``):``}let a=r.match(/^JOIN\((.+)\)$/i);return a?a[1].split(`,`).map(e=>{let r=e.trim();if(r.startsWith(`"`)&&r.endsWith(`"`))return r.slice(1,-1);let i=t.fields.find(e=>e.name.toLowerCase()===r.toLowerCase());return i?String(n.values[i.id]??``):r}).join(``):``}function v(e){return e==null?``:String(e)}function ce(e,t){let n=t.query.trim();if(!n)return e.records;let r=n.toLowerCase();return e.records.filter(n=>(t.fieldId===`all`?e.fields.map(e=>e.id):[t.fieldId]).some(e=>v(n.values[e]).toLowerCase().includes(r)))}function y(e,t,n){let r=n===`asc`?1:-1;return{...e,records:[...e.records].sort((e,n)=>{let i=v(e.values[t]),a=v(n.values[t]),o=Number(i),s=Number(a);return!Number.isNaN(o)&&!Number.isNaN(s)?(o-s)*r:i.localeCompare(a,void 0,{numeric:!0,sensitivity:`base`})*r})}}function le(e,t){let n=new Map;return e.records.forEach(e=>{let r=v(e.values[t]).trim().toLowerCase();r&&n.set(r,(n.get(r)??0)+1)}),e.records.filter(e=>{let r=v(e.values[t]).trim().toLowerCase();return r?(n.get(r)??0)>1:!1})}function ue(e,t){return e.records.filter(e=>!v(e.values[t]).trim())}function de(e,t){if(!t.find)return[];let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=new RegExp(r,n),a=new Set(t.recordIds??e.records.map(e=>e.id)),o=[];return e.records.forEach(e=>{a.has(e.id)&&t.fieldIds.forEach(n=>{let r=v(e.values[n]),a=r.replace(i,t.replacement);a!==r&&o.push({recordId:e.id,fieldId:n,before:r,after:a})})}),o}function fe(e,t){if(!t.find)return{table:e,count:0};let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=new RegExp(r,n),a=new Set(t.recordIds??e.records.map(e=>e.id)),o=0,s=e.records.map(e=>{if(!a.has(e.id))return e;let n=!1,r={...e.values};return t.fieldIds.forEach(e=>{let a=v(r[e]),s=a.replace(i,()=>(o+=1,t.replacement));s!==a&&(n=!0,r[e]=s)}),n?{...e,updatedAt:new Date().toISOString(),values:r}:e});return{table:{...e,records:s},count:o}}function b(e){return e==null?``:String(e).trim().toLowerCase()}function pe(e,n,r,i,a){return{id:t(`relationship`),name:e.trim()||`New Relationship`,fromTableId:n,fromFieldId:r,toTableId:i,toFieldId:a}}function me(e,t){return{...e,updatedAt:new Date().toISOString(),schema:{...e.schema,relationships:[...e.schema.relationships,t]}}}function he(e,t,n,r){let i=b(n.values[e.fromFieldId]);return!i||t.id!==e.fromTableId||r.id!==e.toTableId?[]:r.records.filter(t=>b(t.values[e.toFieldId])===i)}function x(e,t){let n=e.schema.tables.find(e=>e.id===t.fromTableId),r=e.schema.tables.find(e=>e.id===t.toTableId),i=n?.fields.find(e=>e.id===t.fromFieldId),a=r?.fields.find(e=>e.id===t.toFieldId);return`${n?.name??`Table`}:${i?.name??`Field`} -> ${r?.name??`Table`}:${a?.name??`Field`}`}function S(e,t,n,r,i,a){return{id:e,title:t,gradeBand:n,goal:r,table:f(t,i),reflectionQuestions:a}}var C=[S(`classroom-library`,`Classroom Library Catalog`,`Grades 3-8`,`Track books, genres, reading levels, and recommendations.`,`Title,Author,Genre,Pages,Who would enjoy it?
...
Apollo 11 mission,NASA,1969,Space exploration,First humans walked on the Moon`,[`How should the exhibit be grouped?`,`What field would help visitors understand the item?`])];function ge(e){let t=e.table.fields.map(e=>({...e,id:`${e.id}_${Date.now().toString(36)}`})),n=e.table.records.map(n=>r(t,Object.fromEntries(e.table.fields.map((e,r)=>[t[r].id,n.values[e.id]??``]))));return{...e.table,id:`table_${e.id}_${Date.now().toString(36)}`,fields:t,records:n}}var w=document.querySelector(`#app`);if(!w)throw Error(`ListSplatTM app root was not found.`);var T=w,E=ae()??a(),D=E.schema.tables[0].id,O=E.schema.tables[0].records[0]?.id??``,k=`table`,A=`Saved locally`,j=``,M=`all`,N=``,P=`asc`,F=new Set,I=`none`,L=``,R=`Tip: Start with one table, then add relationships when your project needs them.`,z=[],B=[];function V(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function H(){return E.schema.tables.find(e=>e.id===D)??E.schema.tables[0]}function U(e){let t=ce(e,{query:j,fieldId:M});return F.size>0?t.filter(e=>F.has(e.id)):t}function W(e){e.records.some(e=>e.id===O)||(O=e.records[0]?.id??``)}function G(e){E=e,W(H()),h(E),A=`Saved locally`,Q()}function K(e){z=[{label:e,project:structuredClone(E)},...z].slice(0,12)}function _e(){let e=z[0];if(!e){R=`Nothing to undo yet.`,Q();return}z=z.slice(1),E=e.project,D=E.schema.tables.some(e=>e.id===D)?D:E.schema.tables[0].id,W(H()),h(E),R=`Undid ${e.label}.`,Q()}function q(e){D=e.id,G(u(E,e))}function J(){document.querySelectorAll(`.menu[open]`).forEach(e=>{e.open=!1})}function ve(e){G({...E,updatedAt:new Date().toISOString(),metadata:{...E.metadata,title:e||`Untitled Database`}})}function ye(){g(`${E.metadata.title||`listsplat-project`}.listsplat.json`,JSON.stringify(E,null,2),`application/json`)}function be(){g(`${H().name}.csv`,ie(H()),`text/csv;charset=utf-8`)}function xe(){let e=H(),t=U(e),n=`<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>${V(E.metadata.title)}</title>
...
<body><h1>${V(E.metadata.title)}</h1><p>${V(e.name)} report from ListSplatTM.</p>
<table><thead><tr>${e.fields.map(e=>`<th>${V(e.name)}</th>`).join(``)}</tr></thead>
<tbody>${t.map(t=>`<tr>${e.fields.map(n=>`<td>${V(Z(e,t,n.id))}</td>`).join(``)}</tr>`).join(``)}</tbody></table></body></html>`;g(`${E.metadata.title||`listsplat-report`}.html`,n,`text/html;charset=utf-8`)}function Se(e){e.text().then(e=>{let t=JSON.parse(e);d(t),D=t.schema.tables[0].id,O=t.schema.tables[0].records[0]?.id??``,G(t)}).catch(e=>{window.alert(e instanceof Error?e.message:`Could not open this ListSplatTM file.`)})}function Ce(e){e.text().then(t=>{let n=f(e.name.replace(/\.csv$/i,``),t);K(`CSV import`),D=n.id,O=n.records[0]?.id??``,R=`Imported ${n.records.length} records from ${e.name}.`,G({...E,updatedAt:new Date().toISOString(),schema:{...E.schema,tables:[...E.schema.tables,n]},layouts:[...E.layouts,{id:`layout_${Date.now().toString(36)}`,name:`${n.name} Table`,tableId:n.id,mode:`table`,locked:!1}]})})}function we(e){let t=C.find(t=>t.id===e);if(!t)return;let n=ge(t);K(`template load`),D=n.id,O=n.records[0]?.id??``,R=`Loaded ${t.title}.`,G({...E,metadata:{...E.metadata,title:t.title},schema:{...E.schema,tables:[...E.schema.tables,n]},teacher:{...E.teacher,notes:t.reflectionQuestions}})}function Y(e,t){return`
    <details class="menu">
      <summary>${e}</summary>
  ...
  `}function X(e=`text`){return[[`text`,`Short text`],[`longText`,`Long text`],[`number`,`Number`],[`currency`,`Currency`],[`percent`,`Percent`],[`date`,`Date`],[`checkbox`,`Checkbox`],[`choice`,`Choice`],[`image`,`Image`],[`link`,`Link`],[`calculation`,`Calculation`]].map(([t,n])=>`<option value="${t}" ${e===t?`selected`:``}>${n}</option>`).join(``)}function Z(e,t,n){let r=e.fields.find(e=>e.id===n);return r?.type===`calculation`&&r.formula?se(r.formula,e,t):t.values[n]??``}function Te(e,t,n,r){let i=e.fields.find(e=>e.id===n),a=Z(e,t,n),o=`aria-label="${V(i?.name??`Field`)}, record ${r+1}" data-record-id="${t.id}" data-field-id="${n}"`;return i?.type===`checkbox`?`<input class="cell-checkbox" type="checkbox" ${o} ${a===!0||a===`true`?`checked`:``}>`:i?.type===`longText`?`<textarea class="cell-input" ${o}>${V(a)}</textarea>`:i?.type===`date`?`<input class="cell-input" type="date" ${o} value="${V(a)}">`:i?.type===`number`||i?.type===`currency`||i?.type===`percent`?`<input class="cell-input" type="number" step="any" ${o} value="${V(a)}">`:i?.type===`calculation`?`<output class="calc-output">${V(a)}</output>`:`<input class="cell-input" ${o} value="${V(a)}">`}function Ee(e){return`
    <div class="table-tabs">
      ${E.schema.tables.map(t=>`<button type="button" class="table-tab ${t.id===e.id?`active`:``}" data-table-id="${t.id}">${V(t.name)}</button>`).join(``)}
    </div>
        ...
        <tbody>
          ${t.map((t,n)=>`
                <tr class="${t.id===O?`active-row`:``}" data-record-row="${t.id}">
                  ...
                  ${e.fields.filter(e=>!e.hidden).map(r=>`<td>${Te(e,t,r.id,n)}</td>`).join(``)}
  ...
  `}function Oe(e){let t=e.records.find(e=>e.id===O)??e.records[0];return t?`
    <div class="form-view">
      <div class="form-nav">
  ...
  `:`<div class="empty-panel">Add a record to use form view.</div>`}function ke(e,t){return`
    <div class="cards-view ${k===`gallery`?`gallery-view`:``}">
      ${t.map(t=>`
            <article class="record-card" data-select-record="${t.id}">
              ${e.fields.filter(e=>!e.hidden).slice(0,k===`gallery`?5:8).map(n=>`<p><strong>${V(n.name)}</strong><span>${V(Z(e,t,n.id))}</span></p>`).join(``)}
            </article>
          `).join(``)}
    </div>
    ...
    <div class="labels-view">
      ${t.map(t=>`
            <article class="print-label">
              ${e.fields.filter(e=>!e.hidden).slice(0,4).map(n=>`<p><strong>${V(n.name)}:</strong> ${V(Z(e,t,n.id))}</p>`).join(``)}
            </article>
          `).join(``)}
    </div>
  `}function je(e,t){let n=_(e);return`
    <div class="report-view">
      <header>
        <h2>${V(E.metadata.title)}</h2>
  ...
  `}function Me(e){let t=U(e),n=k===`form`?Oe(e):k===`cards`||k===`gallery`?ke(e,t):k===`labels`?Ae(e,t):k===`report`?je(e,t):De(e,t);return`
  ...
  `}function Ne(e){let t=_(e),n=e.records.find(e=>e.id===O)??e.records[0],r=n?E.schema.relationships.filter(t=>t.fromTableId===e.id).map(t=>{let r=E.schema.tables.find(e=>e.id===t.toTableId),i=r?he(t,e,n,r).length:0;return`
  ...
  `}function Pe(e){if(I===`none`)return``;if(I===`replace`){let t=B.slice(0,8);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Replace values">
          ...
          ${B.length?`<div class="replace-preview"><strong>${B.length} change${B.length===1?``:`s`} ready</strong>${t.map(e=>`<p><del>${V(e.before)}</del><ins>${V(e.after)}</ins></p>`).join(``)}</div>`:``}
    ...
    `}if(I===`field`){let t=e.fields.find(e=>e.id===L)??e.fields[0];return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Field settings">
          ...
          <label>Type <select data-field-type>${X(t.type)}</select></label>
    ...
    `;if(I===`relationship`){let e=E.schema.tables.map(e=>`<option value="${e.id}">${V(e.name)}</option>`).join(``),t=E.schema.tables.map(e=>e.fields.map(t=>`<option value="${e.id}:${t.id}">${V(e.name)} - ${V(t.name)}</option>`).join(``)).join(``);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Relationships">
          ...
          <label>Parent table <select data-relationship-from-table>${e}</select></label>
          <label>Parent match field <select data-relationship-from-field>${t}</select></label>
          <label>Related table <select data-relationship-to-table>${e}</select></label>
          <label>Related match field <select data-relationship-to-field>${t}</select></label>
          ${E.schema.relationships.length?`<div class="relationship-list">${E.schema.relationships.map(e=>`<p><strong>${V(e.name)}</strong><br>${V(x(E,e))}</p>`).join(``)}</div>`:`<p>No relationships yet. Add a second table first for the most useful results.</p>`}
          <div class="modal-actions">
  ...
  `}function Q(){let e=H();W(e),T.innerHTML=`
    ...
    <main class="listsplat-app">
      <nav class="menu-bar" aria-label="Application menu">
        ...
        ${Y(`Layout`,[[`table-view`,`Table view`],[`form-view`,`Form view`],[`cards-view`,`Card view`],[`gallery-view`,`Gallery view`],[`labels-view`,`Label view`],[`report-view`,`Report view`]])}
      ...
      <section class="toolbar" aria-label="Database toolbar">
        <label>Title <input data-project-title value="${V(E.metadata.title)}"></label>
        <label>Search <input data-search value="${V(j)}" placeholder="Find records"></label>
        <label>In <select data-search-field><option value="all">All fields</option>${e.fields.map(e=>`<option value="${e.id}" ${M===e.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
        <label>Sort <select data-sort-field><option value="">Choose field</option>${e.fields.map(e=>`<option value="${e.id}" ${N===e.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
        ...
        <label>Type <select data-new-field-type>${X()}</select></label>
    ...
    ${Pe(e)}
  `}function Fe(e){return e instanceof HTMLInputElement&&e.type===`checkbox`?e.checked:e instanceof HTMLInputElement&&e.type===`number`?e.value===``?``:Number(e.value):e.value}function Ie(e){let t=e.dataset.recordId,n=e.dataset.fieldId;!t||!n||(E=u(E,l(H(),t,n,Fe(e))),h(E),A=`Saved locally`)}function Le(){let e=H(),t=e.fields.find(e=>e.id===L);if(!t)return;let n=T.querySelector(`[data-field-name]`)?.value??t.name,r=T.querySelector(`[data-field-type]`)?.value??t.type,i=T.querySelector(`[data-field-description]`)?.value??``,a=T.querySelector(`[data-field-required]`)?.checked??!1,o=T.querySelector(`[data-field-hidden]`)?.checked??!1,s=T.querySelector(`[data-field-formula]`)?.value??``;q(c(e,t.id,{name:n,type:r,description:i,required:a,hidden:o,formula:s})),I=`none`,R=`Updated ${n}.`,Q()}function Re(){let e=T.querySelector(`[data-replace-find]`)?.value??``,t=T.querySelector(`[data-replace-with]`)?.value??``,n=T.querySelector(`[data-replace-field]`)?.value??H().fields[0]?.id,r=j?U(H()).map(e=>e.id):void 0;K(`replace`);let i=fe(H(),{fieldIds:[n],find:e,replacement:t,recordIds:r});I=`none`,B=[],R=`Replaced ${i.count} value${i.count===1?``:`s`}.`,q(i.table)}function ze(){let e=T.querySelector(`[data-replace-find]`)?.value??``,t=T.querySelector(`[data-replace-with]`)?.value??``,n=T.querySelector(`[data-replace-field]`)?.value??H().fields[0]?.id,r=j?U(H()).map(e=>e.id):void 0;B=de(H(),{fieldIds:[n],find:e,replacement:t,recordIds:r}),R=`Preview found ${B.length} change${B.length===1?``:`s`}.`,Q()}function $(e){let t=T.querySelector(e)?.value;if(!t)return null;let[n,r]=t.split(`:`);return n&&r?{tableId:n,fieldId:r}:null}function Be(){let e=T.querySelector(`[data-relationship-from-table]`)?.value??``,t=T.querySelector(`[data-relationship-to-table]`)?.value??``,n=$(`[data-relationship-from-field]`),r=$(`[data-relationship-to-field]`),i=T.querySelector(`[data-relationship-name]`)?.value??``;if(!e||!t||!n||!r){R=`Choose both tables and both match fields.`,Q();return}if(n.tableId!==e||r.tableId!==t){R=`Match fields must belong to the tables you chose.`,Q();return}K(`relationship create`);let a=pe(i,e,n.fieldId,t,r.fieldId);R=`Created relationship: ${a.name}.`,G(me(E,a))}T.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action]`)?.dataset.action,r=t.closest(`[data-table-id]`)?.dataset.tableId,i=t.closest(`[data-template-id]`)?.dataset.templateId,c=t.closest(`[data-view-mode]`)?.dataset.viewMode,l=t.closest(`[data-select-record]`)?.dataset.selectRecord,u=t.closest(`[data-field-settings]`)?.dataset.fieldSettings,d=t.closest(`[data-record-action-id]`)?.dataset.recordActionId;if(r){D=r,F=new Set,W(H()),Q();return}if(i){we(i);return}if(c){k=c,Q();return}if(l){O=l,k===`table`&&Q();return}if(u){L=u,I=`field`,Q();return}if(n)if(J(),n===`new`){K(`new database`);let e=a(`Untitled Database`);D=e.schema.tables[0].id,O=e.schema.tables[0].records[0]?.id??``,F=new Set,G(e)}else if(n===`save-json`)ye();else if(n===`open-json`)T.querySelector(`[data-open-json]`)?.click();else if(n===`import-csv`)T.querySelector(`[data-import-csv]`)?.click();else if(n===`export-csv`)be();else if(n===`export-report`||n===`project-packet`)xe();else if(n===`print`)window.print();else if(n===`add-record`)K(`add record`),q(s(H()));else if(n===`add-field`){let e=T.querySelector(`[data-new-field]`),t=T.querySelector(`[data-new-field-type]`)?.value;K(`add field`),q(o(H(),e?.value||`New Field`,t??`text`))}else if(n===`add-table`){let e=window.prompt(`New table name?`,`New Table`)??``;K(`add table`);let t=ne(E,e);D=t.schema.tables.at(-1)?.id??D,O=H().records[0]?.id??``,G(t)}else if(n===`duplicate-record`&&d)K(`duplicate record`),q(ee(H(),d));else if(n===`delete-record`&&d)K(`delete record`),q(te(H(),d));else if(n===`toggle-sort`)P=P===`asc`?`desc`:`asc`,Q();else if(n===`sort`)N&&q(y(H(),N,P));else if(n===`duplicates`){let e=M===`all`?H().fields[0]?.id:M;F=new Set(le(H(),e).map(e=>e.id)),R=`Found ${F.size} duplicate record${F.size===1?``:`s`}.`,Q()}else if(n===`missing`){let e=M===`all`?H().fields[0]?.id:M;F=new Set(ue(H(),e).map(e=>e.id)),R=`Found ${F.size} record${F.size===1?``:`s`} with missing values.`,Q()}else n===`clear-find`?(j=``,F=new Set,R=`Showing all records.`,Q()):n===`replace`?(B=[],I=`replace`,Q()):n===`preview-replace`?ze():n===`run-replace`?Re():n===`save-field-settings`?(K(`field settings`),Le()):n===`create-relationship`?Be():n===`undo-change`?_e():n===`close-dialog`?(I=`none`,B=[],Q()):n.endsWith(`-view`)?(k=n.replace(`-view`,``),Q()):n===`templates`?(R=`Template starters are in the Teacher panel.`,Q()):n===`project-ideas`?(I=`projectIdeas`,Q()):n===`relationships`?(I=`relationship`,Q()):n.startsWith(`help-`)||n===`functions`||n===`quality`?(I=`help`,Q()):(R=`This ListSplatTM tool is planned for a later build.`,Q())}),T.addEventListener(`change`,e=>{let t=e.target;t.matches(`[data-open-json]`)&&t instanceof HTMLInputElement&&t.files?.[0]?Se(t.files[0]):t.matches(`[data-import-csv]`)&&t instanceof HTMLInputElement&&t.files?.[0]?Ce(t.files[0]):t.matches(`[data-search-field]`)?(M=t.value,F=new Set,Q()):t.matches(`[data-sort-field]`)?(N=t.value,N&&q(y(H(),N,P))):t.matches(`.cell-input, .cell-checkbox`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement)&&Ie(t)}),T.addEventListener(`input`,e=>{let t=e.target;if(t.matches(`[data-project-title]`)){ve(t.value);return}if(t.matches(`[data-search]`)){j=t.value,F=new Set,Q();return}t.matches(`.cell-input`)&&Ie(t)}),document.addEventListener(`click`,e=>{e.target.closest(`.menu`)||J()}),Q();