...
`)}var se=`listsplat.autosave.v1`;function f(e){localStorage.setItem(se,JSON.stringify(e))}function ce(){let e=localStorage.getItem(se);if(!e)return null;let t=JSON.parse(e);return d(t),t}function p(e,t,n){let r=new Blob([t],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=e,a.click(),URL.revokeObjectURL(i)}function le(e,t){return e.records.map(e=>Number(e.values[t])).filter(e=>Number.isFinite(e))}function ue(e){return e.fields.filter(e=>[`number`,`currency`,`percent`].includes(e.type)).map(t=>{let n=le(e,t.id),r=n.reduce((e,t)=>e+t,0);return{fieldId:t.id,fieldName:t.name,count:n.length,sum:r,average:n.length?r/n.length:0,minimum:n.length?Math.min(...n):0,maximum:n.length?Math.max(...n):0}})}function de(e){let t=[],n=``,r=!1;for(let i=0;i<e.length;i+=1){let a=e[i],o=e[i-1];if(a===`"`&&o!==`\\`){r=!r,n+=a;continue}if(a===`,`&&!r){t.push(n.trim()),n=``;continue}n+=a}return(n.trim()||e.endsWith(`,`))&&t.push(n.trim()),t}function fe(e){let t=e.trim();if(t.startsWith(`"`)&&t.endsWith(`"`))return t.slice(1,-1).replace(/\\"/g,`"`)}function pe(e,t,n){let r=n.trim().toLowerCase(),i=e.fields.find(e=>e.name.toLowerCase()===r);return i?String(t.values[i.id]??``):``}function m(e,t,n){return fe(n)??pe(e,t,n)}function h(e,t,n){let r=Number(m(e,t,n));return Number.isFinite(r)?r:0}function me(e,t){let n=t.trim().toLowerCase(),r=e.fields.find(e=>e.name.toLowerCase()===n);return r?le(e,r.id):[]}function g(e){return Number.isFinite(e)?String(Number(e.toFixed(8))):`Formula error: not a number`}function he(e){return e.toLowerCase().replace(/\b[a-z]/g,e=>e.toUpperCase())}function ge(e,t,n){let r=e.trim();if(!r)return``;let i=r.match(/^([A-Z_]+)\((.*)\)$/i);if(!i)return`Formula error: use a function like FIELD(Name)`;let a=i[1].toUpperCase(),o=de(i[2]),s=o[0]??``;if(a===`FIELD`)return pe(t,n,s);if(a===`JOIN`)return o.map(e=>m(t,n,e)).join(``);if(a===`UPPER`)return m(t,n,s).toUpperCase();if(a===`LOWER`)return m(t,n,s).toLowerCase();if(a===`TITLECASE`)return he(m(t,n,s));if(a===`TRIM`)return m(t,n,s).trim();if(a===`LENGTH`)return String(m(t,n,s).length);if(a===`CONTAINS`)return m(t,n,s).toLowerCase().includes(m(t,n,o[1]??``).toLowerCase())?`Yes`:`No`;if(a===`IF_EMPTY`)return m(t,n,s).trim()?m(t,n,s):m(t,n,o[1]??``);if(a===`ADD`)return g(o.reduce((e,r)=>e+h(t,n,r),0));if(a===`SUBTRACT`)return g(o.slice(1).reduce((e,r)=>e-h(t,n,r),h(t,n,s)));if(a===`MULTIPLY`)return g(o.reduce((e,r)=>e*h(t,n,r),1));if(a===`DIVIDE`){let e=h(t,n,o[1]??``);return e===0?`Formula error: divide by zero`:g(h(t,n,s)/e)}if(a===`ROUND`){let e=Math.max(0,Math.min(6,Math.round(h(t,n,o[1]??`"0"`))));return String(Number(h(t,n,s).toFixed(e)))}if([`SUM`,`AVERAGE`,`MIN`,`MAX`,`COUNT`].includes(a)){let e=me(t,s);return a===`SUM`?g(e.reduce((e,t)=>e+t,0)):a===`AVERAGE`?g(e.length?e.reduce((e,t)=>e+t,0)/e.length:0):a===`MIN`?g(e.length?Math.min(...e):0):a===`MAX`?g(e.length?Math.max(...e):0):String(e.length)}return`Formula error: ${a} is not supported`}function _(e){return e==null?``:String(e)}function _e(e,t){let n=t.query.trim();if(!n)return e.records;let r=n.toLowerCase();return e.records.filter(n=>(t.fieldId===`all`?e.fields.map(e=>e.id):[t.fieldId]).some(e=>_(n.values[e]).toLowerCase().includes(r)))}function ve(e,t,n){let r=n===`asc`?1:-1;return{...e,records:[...e.records].sort((e,n)=>{let i=_(e.values[t]),a=_(n.values[t]),o=Number(i),s=Number(a);return!Number.isNaN(o)&&!Number.isNaN(s)?(o-s)*r:i.localeCompare(a,void 0,{numeric:!0,sensitivity:`base`})*r})}}function ye(e,t){let n=new Map;return e.records.forEach(e=>{let r=_(e.values[t]).trim().toLowerCase();r&&n.set(r,(n.get(r)??0)+1)}),e.records.filter(e=>{let r=_(e.values[t]).trim().toLowerCase();return r?(n.get(r)??0)>1:!1})}function be(e,t){return e.records.filter(e=>!_(e.values[t]).trim())}function xe(e,t){if(!t.find)return[];let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=t.wholeWord?`\\b${r}\\b`:r,a=new RegExp(i,n),o=new Set(t.recordIds??e.records.map(e=>e.id)),s=[];return e.records.forEach(e=>{o.has(e.id)&&t.fieldIds.forEach(n=>{let r=_(e.values[n]),i=r.replace(a,t.replacement);i!==r&&s.push({recordId:e.id,fieldId:n,before:r,after:i})})}),s}function Se(e,t){if(!t.find)return{table:e,count:0};let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=t.wholeWord?`\\b${r}\\b`:r,a=new RegExp(i,n),o=new Set(t.recordIds??e.records.map(e=>e.id)),s=0,c=e.records.map(e=>{if(!o.has(e.id))return e;let n=!1,r={...e.values};return t.fieldIds.forEach(e=>{let i=_(r[e]),o=i.replace(a,()=>(s+=1,t.replacement));o!==i&&(n=!0,r[e]=o)}),n?{...e,updatedAt:new Date().toISOString(),values:r}:e});return{table:{...e,records:c},count:s}}function Ce(e){return e==null?``:String(e).trim().toLowerCase()}function we(e,n,r,i,a){return{id:t(`relationship`),name:e.trim()||`New Relationship`,fromTableId:n,fromFieldId:r,toTableId:i,toFieldId:a}}function Te(e,t){return{...e,updatedAt:new Date().toISOString(),schema:{...e.schema,relationships:[...e.schema.relationships,t]}}}function Ee(e,t,n,r){let i=Ce(n.values[e.fromFieldId]);return!i||t.id!==e.fromTableId||r.id!==e.toTableId?[]:r.records.filter(t=>Ce(t.values[e.toFieldId])===i)}function De(e,t){let n=e.schema.tables.find(e=>e.id===t.fromTableId),r=e.schema.tables.find(e=>e.id===t.toTableId),i=n?.fields.find(e=>e.id===t.fromFieldId),a=r?.fields.find(e=>e.id===t.toFieldId);return`${n?.name??`Table`}:${i?.name??`Field`} -> ${r?.name??`Table`}:${a?.name??`Field`}`}function v(e,t,n,r,i,a){return{id:e,title:t,gradeBand:n,goal:r,table:ie(t,i),reflectionQuestions:a}}var Oe=[v(`classroom-library`,`Classroom Library Catalog`,`Grades 3-8`,`Track books, genres, reading levels, and recommendations.`,`Title,Author,Genre,Pages,Who would enjoy it?
...
Apollo 11 mission,NASA,1969,Space exploration,First humans walked on the Moon`,[`How should the exhibit be grouped?`,`What field would help visitors understand the item?`])];function ke(e){let t=e.table.fields.map(e=>({...e,id:`${e.id}_${Date.now().toString(36)}`})),n=e.table.records.map(n=>r(t,Object.fromEntries(e.table.fields.map((e,r)=>[t[r].id,n.values[e.id]??``]))));return{...e.table,id:`table_${e.id}_${Date.now().toString(36)}`,fields:t,records:n}}var Ae=`drawsplat.language`,y=[{code:`en`,label:`English`,dir:`ltr`,htmlLang:`en`},{code:`es`,label:`Español`,dir:`ltr`,htmlLang:`es`},{code:`vi`,label:`Tiếng Việt`,dir:`ltr`,htmlLang:`vi`},{code:`ar`,label:`العربية`,dir:`rtl`,htmlLang:`ar`},{code:`zh`,label:`中文`,dir:`ltr`,htmlLang:`zh`},{code:`uh`,label:`हिन्दी / اردو`,dir:`ltr`,htmlLang:`hi`}],je={es:{New:`Nuevo`,"Save JSON":`Guardar JSON`,"Open JSON":`Abrir JSON`,"Export CSV":`Exportar CSV`,File:`Archivo`,Edit:`Editar`,Data:`Datos`,Layout:`Diseño`,Tools:`Herramientas`,View:`Vista`,Teacher:`Docente`,Help:`Ayuda`,Title:`Título`,Search:`Buscar`,In:`En`,"All fields":`Todos los campos`,Sort:`Ordenar`,"Choose field":`Elegir campo`,"New field":`Campo nuevo`,"Field name":`Nombre del campo`,Type:`Tipo`,"Add field":`Agregar campo`,"Add record":`Agregar registro`,Table:`Tabla`,Form:`Formulario`,Cards:`Tarjetas`,Gallery:`Galería`,Labels:`Etiquetas`,Report:`Informe`,"Upload image":`Subir imagen`,"No image yet":`Sin imagen todavía`},vi:{New:`Mới`,"Save JSON":`Lưu JSON`,"Open JSON":`Mở JSON`,"Export CSV":`Xuất CSV`,File:`Tệp`,Edit:`Sửa`,Data:`Dữ liệu`,Layout:`Bố cục`,Tools:`Công cụ`,View:`Xem`,Teacher:`Giáo viên`,Help:`Trợ giúp`,Title:`Tiêu đề`,Search:`Tìm`,Sort:`Sắp xếp`,Table:`Bảng`,Form:`Biểu mẫu`,Cards:`Thẻ`,Gallery:`Thư viện ảnh`,Labels:`Nhãn`,Report:`Báo cáo`},ar:{New:`جديد`,"Save JSON":`حفظ JSON`,"Open JSON":`فتح JSON`,"Export CSV":`تصدير CSV`,File:`ملف`,Edit:`تحرير`,Data:`بيانات`,Layout:`تخطيط`,Tools:`أدوات`,View:`عرض`,Teacher:`المعلم`,Help:`مساعدة`,Title:`العنوان`,Search:`بحث`,Sort:`فرز`,Table:`جدول`,Form:`نموذج`,Cards:`بطاقات`,Gallery:`معرض`,Labels:`ملصقات`,Report:`تقرير`},zh:{New:`新建`,"Save JSON":`保存 JSON`,"Open JSON":`打开 JSON`,"Export CSV":`导出 CSV`,File:`文件`,Edit:`编辑`,Data:`数据`,Layout:`布局`,Tools:`工具`,View:`视图`,Teacher:`教师`,Help:`帮助`,Title:`标题`,Search:`搜索`,Sort:`排序`,Table:`表格`,Form:`表单`,Cards:`卡片`,Gallery:`图库`,Labels:`标签`,Report:`报告`},uh:{New:`नया`,File:`फ़ाइल`,Edit:`संपादन`,Data:`डेटा`,Layout:`लेआउट`,Tools:`औज़ार`,View:`दृश्य`,Teacher:`शिक्षक`,Help:`सहायता`,Search:`खोज`,Table:`तालिका`,Form:`फ़ॉर्म`,Cards:`कार्ड`,Gallery:`गैलरी`,Labels:`लेबल`,Report:`रिपोर्ट`}},Me=document.querySelector(`#app`);if(!Me)throw Error(`ListSplatTM app root was not found.`);var b=Me,x=ce()??a(),S=x.schema.tables[0].id,C=x.schema.tables[0].records[0]?.id??``,w=`table`,T=`Saved locally`,E=``,D=`all`,O=``,k=`asc`,A=new Set,j=`none`,M=``,N=`Tip: Start with one table, then add relationships when your project needs them.`,P=[],F=[],I=S,L=x.schema.tables[1]?.id??S,R=null,z=``,B=Pe();function V(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function Ne(e){let t=(e||``).toLowerCase();return t.startsWith(`es`)?`es`:t.startsWith(`vi`)?`vi`:t.startsWith(`ar`)?`ar`:t.startsWith(`zh`)?`zh`:t===`uh`||t.startsWith(`ur`)||t.startsWith(`hi`)?`uh`:`en`}function Pe(){let e=new URLSearchParams(window.location.search);try{return Ne(e.get(`lang`)||localStorage.getItem(Ae)||navigator.language)}catch{return Ne(e.get(`lang`)||navigator.language)}}function Fe(e){return y.map(({code:t,label:n})=>`<option value="${t}"${t===e?` selected`:``}>${n}</option>`).join(``)}function H(e){return B===`en`?e:je[B][e]??e}function Ie(){let e=y.find(e=>e.code===B)??y[0];document.documentElement.lang=e.htmlLang,document.documentElement.dir=e.dir}function U(){return x.schema.tables.find(e=>e.id===S)??x.schema.tables[0]}function W(e){let t=_e(e,{query:E,fieldId:D});return A.size>0?t.filter(e=>A.has(e.id)):t}function G(e){e.records.some(e=>e.id===C)||(C=e.records[0]?.id??``)}function K(e){x=e,G(U()),f(x),T=`Saved locally`,$()}function q(e){P=[{label:e,project:structuredClone(x)},...P].slice(0,12)}function Le(){let e=P[0];if(!e){N=`Nothing to undo yet.`,$();return}P=P.slice(1),x=e.project,S=x.schema.tables.some(e=>e.id===S)?S:x.schema.tables[0].id,G(U()),f(x),N=`Undid ${e.label}.`,$()}function J(e){S=e.id,K(u(x,e))}function Re(){document.querySelectorAll(`.menu[open]`).forEach(e=>{e.open=!1})}function ze(e){K({...x,updatedAt:new Date().toISOString(),metadata:{...x.metadata,title:e||`Untitled Database`}})}function Be(){p(`${x.metadata.title||`listsplat-project`}.listsplat.json`,JSON.stringify(x,null,2),`application/json`)}function Ve(){p(`${U().name}.csv`,oe(U()),`text/csv;charset=utf-8`)}function He(){let e=U(),t=W(e),n=`<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>${V(x.metadata.title)}</title>
...
<body><h1>${V(x.metadata.title)}</h1><p>${V(e.name)} report from ListSplatTM.</p>
<table><thead><tr>${e.fields.map(e=>`<th>${V(e.name)}</th>`).join(``)}</tr></thead>
<tbody>${t.map(t=>`<tr>${e.fields.map(n=>`<td>${V(X(e,t,n.id))}</td>`).join(``)}</tr>`).join(``)}</tbody></table></body></html>`;p(`${x.metadata.title||`listsplat-report`}.html`,n,`text/html;charset=utf-8`)}function Ue(e){e.text().then(e=>{let t=JSON.parse(e);d(t),S=t.schema.tables[0].id,C=t.schema.tables[0].records[0]?.id??``,K(t)}).catch(e=>{window.alert(e instanceof Error?e.message:`Could not open this ListSplatTM file.`)})}function We(e){e.text().then(t=>{let n=ie(e.name.replace(/\.csv$/i,``),t);R=n,z=e.name,j=`csvImport`,N=`Previewing ${n.records.length} CSV record${n.records.length===1?``:`s`} from ${e.name}.`,$()})}function Ge(e){if(!R){j=`none`;return}if(q(`CSV import`),e===`new`){let e=R;S=e.id,C=e.records[0]?.id??``,R=null,j=`none`,N=`Imported ${e.records.length} records from ${z}.`,K({...x,updatedAt:new Date().toISOString(),schema:{...x.schema,tables:[...x.schema.tables,e]},layouts:[...x.layouts,{id:`layout_${Date.now().toString(36)}`,name:`${e.name} Table`,tableId:e.id,mode:`table`,locked:!1}]});return}let t=U(),n=new Map(R.fields.map(e=>[e.name.trim().toLowerCase(),e.id])),i=R.records.map(e=>r(t.fields,Object.fromEntries(t.fields.map(t=>{let r=n.get(t.name.trim().toLowerCase());return[t.id,r?e.values[r]??``:``]}))));R=null,j=`none`,N=`Appended ${i.length} CSV record${i.length===1?``:`s`} to ${t.name}.`,J({...t,records:[...t.records,...i]})}function Ke(e){let t=Oe.find(t=>t.id===e);if(!t)return;let n=ke(t);q(`template load`),S=n.id,C=n.records[0]?.id??``,N=`Loaded ${t.title}.`,K({...x,metadata:{...x.metadata,title:t.title},schema:{...x.schema,tables:[...x.schema.tables,n]},teacher:{...x.teacher,notes:t.reflectionQuestions}})}function Y(e,t){return`
    <details class="menu">
  ...
  `}function qe(e=`text`){return[[`text`,`Short text`],[`longText`,`Long text`],[`number`,`Number`],[`currency`,`Currency`],[`percent`,`Percent`],[`date`,`Date`],[`checkbox`,`Checkbox`],[`rating`,`Rating`],[`choice`,`Choice`],[`image`,`Image`],[`link`,`Link`],[`calculation`,`Calculation`],[`autoNumber`,`Auto number`],[`createdAt`,`Created time`],[`updatedAt`,`Updated time`]].map(([t,n])=>`<option value="${t}" ${e===t?`selected`:``}>${n}</option>`).join(``)}function X(e,t,n){let r=e.fields.find(e=>e.id===n);return r?.type===`calculation`&&r.formula?ge(r.formula,e,t):t.values[n]??``}function Z(){return x.layouts.find(e=>e.tableId===S&&e.mode===w)}function Je(e){return!!e}function Q(e){let t=Z()?.fieldOrder??e.fields.map(e=>e.id),n=new Map(e.fields.map(e=>[e.id,e]));return[...t.map(e=>n.get(e)).filter(Je),...e.fields.filter(e=>!t.includes(e.id))].filter(e=>e&&!e.hidden)}function Ye(e){return Q(e).filter(e=>e.type===`image`)}function Xe(e,t){let n=Ye(e)[0];return n?String(X(e,t,n.id)??``):``}function Ze(e){let t=Z();t&&K({...x,updatedAt:new Date().toISOString(),layouts:x.layouts.map(n=>n.id===t.id?{...n,...e}:n)})}function Qe(e,t){let n=e.fields.find(e=>!e.hidden)??e.fields[0],r=n?X(e,t,n.id):``;return String(r||`Untitled record`)}function $e(e,t,n,r){let i=e.fields.find(e=>e.id===n),a=X(e,t,n),o=`aria-label="${V(i?.name??`Field`)}, record ${r+1}" data-record-id="${t.id}" data-field-id="${n}"`;if(i?.type===`checkbox`)return`<input class="cell-checkbox" type="checkbox" ${o} ${a===!0||a===`true`?`checked`:``}>`;if(i?.type===`image`){let e=String(a??``);return`
  ...
  `}function tt(e,t){return`
    <div class="data-grid-wrap">
      <table class="data-grid">
        ...
        <tbody>
          ${t.map((t,n)=>`
                <tr class="${t.id===C?`active-row`:``}" data-record-row="${t.id}">
                  <td><button type="button" class="row-button" data-select-record="${t.id}">${n+1}</button></td>
                  ${Q(e).map(r=>`<td>${$e(e,t,r.id,n)}</td>`).join(``)}
  ...
  `}function nt(e){let t=e.records.find(e=>e.id===C)??e.records[0];if(!t)return`<div class="empty-panel">Add a record to use form view.</div>`;let n=x.schema.relationships.filter(t=>t.fromTableId===e.id).map(n=>{let r=x.schema.tables.find(e=>e.id===n.toTableId),i=r?Ee(n,e,t,r):[];return`
  ...
  `}function rt(e,t){let n=(t,n)=>{let r=X(e,n,t.id);if(t.type===`image`){let e=String(r??``);return`
    ...
    <div class="cards-view ${w===`gallery`?`gallery-view`:``}">
      ${t.map(t=>{let r=Xe(e,t);return`
            <article class="record-card" data-select-record="${t.id}">
              ${w===`gallery`?`<div class="gallery-image">${r?`<img src="${V(r)}" alt="">`:`<span>Add an image field, then upload a picture.</span>`}</div>`:``}
  ...
  `}function at(e,t){let n=ue(e);return`
    <div class="report-view">
      <header>
        <h2>${V(x.metadata.title)}</h2>
  ...
  `}function ot(e){let t=W(e),n={table:`Table: spreadsheet-like rows and columns for fast data entry.`,form:`Form: focus on one record at a time.`,cards:`Cards: compact text-first record cards for browsing.`,gallery:`Gallery: image-first cards for collections and exhibits.`,labels:`Labels: printable small cards or shelf labels.`,report:`Report: printable table with title and summaries.`},r=w===`form`?nt(e):w===`cards`||w===`gallery`?rt(e,t):w===`labels`?it(e,t):w===`report`?at(e,t):tt(e,t);return`
    <section class="database-panel" aria-label="Database table">
      ${et(e)}
      <div class="view-tabs" role="group" aria-label="Layout modes">
        ${[`table`,`form`,`cards`,`gallery`,`labels`,`report`].map(e=>`<button type="button" class="${w===e?`active`:``}" data-view-mode="${e}" title="${V(n[e])}" aria-label="${V(n[e])}">${V(H(e[0].toUpperCase()+e.slice(1)))}</button>`).join(``)}
      </div>
      ${r}
    </section>
  `}function st(e){let t=ue(e),n=e.records.find(e=>e.id===C)??e.records[0],r=n?x.schema.relationships.filter(t=>t.fromTableId===e.id).map(t=>{let r=x.schema.tables.find(e=>e.id===t.toTableId),i=r?Ee(t,e,n,r).length:0;return`
  ...
  `}function ct(e){if(j===`none`)return``;if(j===`replace`){let t=F.slice(0,8);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Replace values">
          ...
          ${F.length?`<div class="replace-preview"><strong>${F.length} change${F.length===1?``:`s`} ready</strong>${t.map(t=>{let n=e.records.find(e=>e.id===t.recordId),r=e.fields.find(e=>e.id===t.fieldId);return`<p><span>${V(n?Qe(e,n):`Record`)} / ${V(r?.name??`Field`)}</span><del>${V(t.before)}</del><ins>${V(t.after)}</ins></p>`}).join(``)}</div>`:``}
    ...
    `}if(j===`field`){let t=e.fields.find(e=>e.id===M)??e.fields[0];return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Field settings">
          ...
          <label>Type <select data-field-type>${qe(t.type)}</select></label>
    ...
    `}if(j===`layout`){let t=Z(),n=Q(e);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Layout designer">
          <h2>Layout designer</h2>
          <p>Arrange fields for the current ${V(w)} view. Locked layouts can still be viewed, but students should not change them.</p>
    ...
    `}if(j===`csvImport`&&R){let t=R.records.slice(0,5);return`
      <div class="modal-backdrop">
        <section class="modal wide-modal" role="dialog" aria-modal="true" aria-label="CSV import preview">
          <h2>CSV import preview</h2>
          <p>${V(z)} has ${R.fields.length} field${R.fields.length===1?``:`s`} and ${R.records.length} record${R.records.length===1?``:`s`}.</p>
          <div class="preview-table-wrap">
            <table class="preview-table">
              <thead>
                <tr>${R.fields.map(e=>`<th>${V(e.name)}</th>`).join(``)}</tr>
              </thead>
              <tbody>
                ${t.map(e=>`<tr>${R.fields.map(t=>`<td>${V(e.values[t.id])}</td>`).join(``)}</tr>`).join(``)}
              </tbody>
            </table>
          </div>
    ...
    `;if(j===`relationship`){let t=x.schema.tables.find(e=>e.id===I)??e,n=x.schema.tables.find(e=>e.id===L)??e,r=t.fields.map(e=>`<option value="${t.id}:${e.id}">${V(e.name)}</option>`).join(``),i=n.fields.map(e=>`<option value="${n.id}:${e.id}">${V(e.name)}</option>`).join(``);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Relationships">
          ...
          <label>Parent table <select data-relationship-from-table>${x.schema.tables.map(e=>`<option value="${e.id}" ${e.id===t.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
          <label>Parent match field <select data-relationship-from-field>${r}</select></label>
          <label>Related table <select data-relationship-to-table>${x.schema.tables.map(e=>`<option value="${e.id}" ${e.id===n.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
          <label>Related match field <select data-relationship-to-field>${i}</select></label>
          ${x.schema.relationships.length?`<div class="relationship-list">${x.schema.relationships.map(e=>`<p><strong>${V(e.name)}</strong><br>${V(De(x,e))}</p>`).join(``)}</div>`:`<p>No relationships yet. Add a second table first for the most useful results.</p>`}
          <div class="modal-actions">
  ...
  `}function $(){let e=U();G(e),Ie(),b.innerHTML=`
    <header class="app-header">
      ...
      <div class="quick-actions">
        ...
        <select id="languageSwitcher" class="lang-switcher" aria-label="Language">${Fe(B)}</select>
      </div>
    </header>
    <main class="listsplat-app">
      ...
      <section class="toolbar" aria-label="Database toolbar">
        <label>${V(H(`Title`))} <input data-project-title value="${V(x.metadata.title)}"></label>
        <label>${V(H(`Search`))} <input data-search value="${V(E)}" placeholder="${V(H(`Find records`))}"></label>
        <label>${V(H(`In`))} <select data-search-field><option value="all">${V(H(`All fields`))}</option>${e.fields.map(e=>`<option value="${e.id}" ${D===e.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
        <label>${V(H(`Sort`))} <select data-sort-field><option value="">${V(H(`Choose field`))}</option>${e.fields.map(e=>`<option value="${e.id}" ${O===e.id?`selected`:``}>${V(e.name)}</option>`).join(``)}</select></label>
        <button type="button" data-action="toggle-sort">${k===`asc`?`A-Z`:`Z-A`}</button>
        <label>${V(H(`New field`))} <input data-new-field placeholder="${V(H(`Field name`))}"></label>
        <label>${V(H(`Type`))} <select data-new-field-type>${qe()}</select></label>
    ...
    ${ct(e)}
  `}function lt(e){return e instanceof HTMLInputElement&&e.type===`checkbox`?e.checked:e instanceof HTMLInputElement&&e.type===`number`?e.value===``?``:Number(e.value):e.value}function ut(e){let t=e.dataset.recordId,n=e.dataset.fieldId;!t||!n||(x=u(x,l(U(),t,n,lt(e))),f(x),T=`Saved locally`)}function dt(e,t,n,r){if(!n.type.startsWith(`image/`)){N=`That clipboard item is not an image.`,$();return}let i=new FileReader;i.addEventListener(`load`,()=>{q(r),x=u(x,l(U(),e,t,String(i.result??``))),f(x),N=`Image saved in this field.`,$()}),i.readAsDataURL(n)}function ft(){let e=U(),t=e.fields.find(e=>e.id===M);if(!t)return;let n=b.querySelector(`[data-field-name]`)?.value??t.name,r=b.querySelector(`[data-field-type]`)?.value??t.type,i=b.querySelector(`[data-field-description]`)?.value??``,a=b.querySelector(`[data-field-required]`)?.checked??!1,o=b.querySelector(`[data-field-hidden]`)?.checked??!1,s=b.querySelector(`[data-field-formula]`)?.value??``,ee=(b.querySelector(`[data-field-options]`)?.value??``).split(`,`).map(e=>e.trim()).filter(Boolean);J(c(e,t.id,{name:n,type:r,description:i,required:a,hidden:o,formula:s,options:ee})),j=`none`,N=`Updated ${n}.`,$()}function pt(){let e=b.querySelector(`[data-replace-find]`)?.value??``,t=b.querySelector(`[data-replace-with]`)?.value??``,n=b.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=b.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=b.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=E?W(U()).map(e=>e.id):void 0;q(`replace`);let o=Se(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i});j=`none`,F=[],N=`Replaced ${o.count} value${o.count===1?``:`s`}.`,J(o.table)}function mt(){let e=b.querySelector(`[data-replace-find]`)?.value??``,t=b.querySelector(`[data-replace-with]`)?.value??``,n=b.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=b.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=b.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=E?W(U()).map(e=>e.id):void 0;F=xe(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i}),N=`Preview found ${F.length} change${F.length===1?``:`s`}.`,$()}function ht(e){let t=b.querySelector(e)?.value;if(!t)return null;let[n,r]=t.split(`:`);return n&&r?{tableId:n,fieldId:r}:null}function gt(){let e=b.querySelector(`[data-relationship-from-table]`)?.value??``,t=b.querySelector(`[data-relationship-to-table]`)?.value??``,n=ht(`[data-relationship-from-field]`),r=ht(`[data-relationship-to-field]`),i=b.querySelector(`[data-relationship-name]`)?.value??``;if(!e||!t||!n||!r){N=`Choose both tables and both match fields.`,$();return}if(n.tableId!==e||r.tableId!==t){N=`Match fields must belong to the tables you chose.`,$();return}q(`relationship create`);let a=we(i,e,n.fieldId,t,r.fieldId);N=`Created relationship: ${a.name}.`,K(Te(x,a))}function _t(){I=b.querySelector(`[data-relationship-from-table]`)?.value??I,L=b.querySelector(`[data-relationship-to-table]`)?.value??L,$()}b.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action]`)?.dataset.action,r=t.closest(`[data-table-id]`)?.dataset.tableId,i=t.closest(`[data-template-id]`)?.dataset.templateId,c=t.closest(`[data-view-mode]`)?.dataset.viewMode,l=t.closest(`[data-select-record]`)?.dataset.selectRecord,u=t.closest(`[data-field-settings]`)?.dataset.fieldSettings,d=t.closest(`[data-record-action-id]`)?.dataset.recordActionId;if(r){S=r,A=new Set,G(U()),$();return}if(i){Ke(i);return}if(c){w=c,$();return}if(l){C=l,w===`table`&&$();return}if(u){M=u,j=`field`,$();return}if(n)if(Re(),n===`new`){q(`new database`);let e=a(`Untitled Database`);S=e.schema.tables[0].id,C=e.schema.tables[0].records[0]?.id??``,A=new Set,K(e)}else if(n===`save-json`)Be();else if(n===`open-json`)b.querySelector(`[data-open-json]`)?.click();else if(n===`import-csv`)b.querySelector(`[data-import-csv]`)?.click();else if(n===`export-csv`)Ve();else if(n===`export-report`||n===`project-packet`)He();else if(n===`print`)window.print();else if(n===`add-record`)q(`add record`),J(s(U()));else if(n===`add-field`){let e=b.querySelector(`[data-new-field]`),t=b.querySelector(`[data-new-field-type]`)?.value;q(`add field`),J(o(U(),e?.value||`New Field`,t??`text`))}else if(n===`add-table`){let e=window.prompt(`New table name?`,`New Table`)??``;q(`add table`);let t=ne(x,e);S=t.schema.tables.at(-1)?.id??S,C=U().records[0]?.id??``,K(t)}else if(n===`duplicate-record`&&d)q(`duplicate record`),J(ee(U(),d));else if(n===`delete-record`&&d)q(`delete record`),J(te(U(),d));else if(n===`toggle-sort`)k=k===`asc`?`desc`:`asc`,$();else if(n===`sort`)O&&J(ve(U(),O,k));else if(n===`duplicates`){let e=D===`all`?U().fields[0]?.id:D;A=new Set(ye(U(),e).map(e=>e.id)),N=`Found ${A.size} duplicate record${A.size===1?``:`s`}.`,$()}else if(n===`missing`){let e=D===`all`?U().fields[0]?.id:D;A=new Set(be(U(),e).map(e=>e.id)),N=`Found ${A.size} record${A.size===1?``:`s`} with missing values.`,$()}else if(n===`clear-find`)E=``,A=new Set,N=`Showing all records.`,$();else if(n===`replace`)F=[],j=`replace`,$();else if(n===`preview-replace`)mt();else if(n===`run-replace`)pt();else if(n===`apply-csv-new`)Ge(`new`);else if(n===`apply-csv-append`)Ge(`append`);else if(n===`save-field-settings`)q(`field settings`),ft();else if(n===`layout-designer`||n===`lock-layout`)j=`layout`,$();else if(n===`layout-field-up`||n===`layout-field-down`){let e=t.closest(`[data-layout-field-id]`)?.dataset.layoutFieldId,r=Z();if(e&&r){let t=Q(U()).map(e=>e.id),r=t.indexOf(e),i=n===`layout-field-up`?r-1:r+1;r>=0&&i>=0&&i<t.length&&([t[r],t[i]]=[t[i],t[r]],q(`layout order`),Ze({fieldOrder:t}),j=`layout`)}}else if(n===`save-layout-settings`){let e=b.querySelector(`[data-layout-locked]`)?.checked??!1;q(`layout settings`),Ze({locked:e,fieldOrder:Q(U()).map(e=>e.id)}),j=`none`}else n===`create-relationship`?gt():n===`undo-change`?Le():n===`close-dialog`?(j=`none`,F=[],R=null,$()):n.endsWith(`-view`)?(w=n.replace(`-view`,``),$()):n===`templates`?(N=`Template starters are in the Teacher panel.`,$()):n===`project-ideas`?(j=`projectIdeas`,$()):n===`relationships`?(j=`relationship`,$()):n.startsWith(`help-`)||n===`functions`||n===`quality`?(j=`help`,$()):(N=`This ListSplatTM tool is planned for a later build.`,$())}),b.addEventListener(`change`,e=>{let t=e.target;if(t.matches(`#languageSwitcher`)&&t instanceof HTMLSelectElement){B=Ne(t.value);try{localStorage.setItem(Ae,B)}catch{}$()}else if(t.matches(`[data-open-json]`)&&t instanceof HTMLInputElement&&t.files?.[0])Ue(t.files[0]);else if(t.matches(`[data-import-csv]`)&&t instanceof HTMLInputElement&&t.files?.[0])We(t.files[0]);else if(t.matches(`[data-search-field]`))D=t.value,A=new Set,$();else if(t.matches(`[data-sort-field]`))O=t.value,O&&J(ve(U(),O,k));else if(t.matches(`[data-relationship-from-table], [data-relationship-to-table]`))_t();else if(t.matches(`.cell-input, .cell-checkbox, .image-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement))if(t instanceof HTMLInputElement&&t.type===`file`&&t.files?.[0]){let e=t.dataset.recordId,n=t.dataset.fieldId,r=t.files[0];e&&n&&dt(e,n,r,`image upload`)}else ut(t)}),b.addEventListener(`paste`,e=>{let t=e.target.closest(`.image-cell`);if(!t)return;let n=t.dataset.recordId,r=t.dataset.fieldId,i=Array.from(e.clipboardData?.items??[]).find(e=>e.type.startsWith(`image/`))?.getAsFile();n&&r&&i&&(e.preventDefault(),dt(n,r,i,`image paste`))}),b.addEventListener(`input`,e=>{let t=e.target;if(t.matches(`[data-project-title]`)){ze(t.value);return}if(t.matches(`[data-search]`)){E=t.value,A=new Set,$();return}t.matches(`.cell-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement)&&ut(t)}),document.addEventListener(`click`,e=>{e.target.closest(`.menu`)||Re()}),document.addEventListener(`toggle`,e=>{let t=e.target;!(t instanceof HTMLDetailsElement)||!t.matches(`.menu`)||!t.open||document.querySelectorAll(`.menu[open]`).forEach(e=>{e!==t&&(e.open=!1)})},!0),$();