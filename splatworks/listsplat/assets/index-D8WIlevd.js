...
<tbody>${t.map(t=>`<tr>${e.fields.map(n=>`<td>${V(X(e,t,n.id))}</td>`).join(``)}</tr>`).join(``)}</tbody></table></body></html>`;m(`${w.metadata.title||`listsplat-report`}.html`,n,`text/html;charset=utf-8`)}function Ge(){let e=w.schema.tables.map(e=>{let t=et(e),n=t.reduce((e,t)=>e+t.missing,0),r=t.reduce((e,t)=>e+t.duplicates,0);return`
        <section>
          <h2>${V(e.name)}</h2>
      ...
      `}).join(``),t=w.schema.relationships.length?`<ul>${w.schema.relationships.map(e=>`<li>${V(e.name)}: ${V(de(w,e))}</li>`).join(``)}</ul>`:`<p>No relationships have been created yet.</p>`,n=w.teacher.notes.length?`<ul>${w.teacher.notes.map(e=>`<li>${V(e)}</li>`).join(``)}</ul>`:`<p>No teacher notes yet.</p>`,r=`<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><title>${V(w.metadata.title)} Project Packet</title>
...
<body>
  <h1>${V(w.metadata.title||`ListSplat Project`)} Project Packet</h1>
  <div class="meta">
    <div><strong>Author</strong><br>${V(w.metadata.author||`Not set`)}</div>
    <div><strong>Class</strong><br>${V(w.metadata.className||`Not set`)}</div>
    <div><strong>Tables</strong><br>${w.schema.tables.length}</div>
    <div><strong>Relationships</strong><br>${w.schema.relationships.length}</div>
  </div>
  <section><h2>Teacher Notes</h2>${n}</section>
  <section><h2>Relationships</h2>${t}</section>
  ${e}
</body></html>`;m(`${w.metadata.title||`listsplat`}-project-packet.html`,r,`text/html;charset=utf-8`)}function Ke(e){e.text().then(e=>{let t=JSON.parse(e);d(t),T=t.schema.tables[0].id,E=t.schema.tables[0].records[0]?.id??``,K(t)}).catch(e=>{window.alert(e instanceof Error?e.message:`Could not open this ListSplatTM file.`)})}function qe(e){e.text().then(t=>{let n=re(e.name.replace(/\.csv$/i,``),t);z=n,Pe=e.name,P=`csvImport`,F=`Previewing ${n.records.length} CSV record${n.records.length===1?``:`s`} from ${e.name}.`,$()})}function Je(e){if(!z){P=`none`;return}if(q(`CSV import`),e===`new`){let e=z;T=e.id,E=e.records[0]?.id??``,z=null,P=`none`,F=`Imported ${e.records.length} records from ${Pe}.`,K({...w,updatedAt:new Date().toISOString(),schema:{...w.schema,tables:[...w.schema.tables,e]},layouts:[...w.layouts,{id:`layout_${Date.now().toString(36)}`,name:`${e.name} Table`,tableId:e.id,mode:`table`,locked:!1}]});return}let t=U(),n=new Map(z.fields.map(e=>[e.name.trim().toLowerCase(),e.id])),i=z.records.map(e=>r(t.fields,Object.fromEntries(t.fields.map(t=>{let r=n.get(t.name.trim().toLowerCase());return[t.id,r?e.values[r]??``:``]}))));z=null,P=`none`,F=`Appended ${i.length} CSV record${i.length===1?``:`s`} to ${t.name}.`,J({...t,records:[...t.records,...i]})}function Ye(e){let t=Ee.find(t=>t.id===e);if(!t)return;let n=De(t);q(`template load`),T=n.id,E=n.records[0]?.id??``,F=`Loaded ${t.title}.`,K({...w,metadata:{...w.metadata,title:t.title},schema:{...w.schema,tables:[...w.schema.tables,n]},teacher:{...w.teacher,notes:t.reflectionQuestions}})}function Y(e,t){return`
    <details class="menu">
  ...
  `}function Xe(e=`text`){return[[`text`,`Short text`],[`longText`,`Long text`],[`number`,`Number`],[`currency`,`Currency`],[`percent`,`Percent`],[`date`,`Date`],[`checkbox`,`Checkbox`],[`rating`,`Rating`],[`choice`,`Choice`],[`image`,`Image`],[`link`,`Link`],[`calculation`,`Calculation`],[`autoNumber`,`Auto number`],[`createdAt`,`Created time`],[`updatedAt`,`Updated time`]].map(([t,n])=>`<option value="${t}" ${e===t?`selected`:``}>${n}</option>`).join(``)}function X(e,t,n){let r=e.fields.find(e=>e.id===n);return r?.type===`calculation`&&r.formula?be(r.formula,e,t,w):t.values[n]??``}function Z(){return w.layouts.find(e=>e.tableId===T&&e.mode===D)}function Ze(e){return!!e}function Q(e){let t=Z()?.fieldOrder??e.fields.map(e=>e.id),n=new Map(e.fields.map(e=>[e.id,e]));return[...t.map(e=>n.get(e)).filter(Ze),...e.fields.filter(e=>!t.includes(e.id))].filter(e=>e&&!e.hidden)}function Qe(e){return e.fields.filter(e=>e.type===`calculation`)}function $e(e){return Qe(e).reduce((t,n)=>n.formula?t+e.records.filter(t=>String(be(n.formula??``,e,t,w)).startsWith(`Formula error:`)).length:t,0)}function et(e){return e.fields.filter(e=>!e.hidden&&![`image`,`autoNumber`,`createdAt`,`updatedAt`,`calculation`].includes(e.type)).map(t=>({field:t,missing:x(e,t.id).length,duplicates:Ce(e,t.id).length}))}function tt(e){return Q(e).filter(e=>e.type===`image`)}function nt(e,t){let n=tt(e)[0];return n?String(X(e,t,n.id)??``):``}function rt(e){let t=Z();t&&K({...w,updatedAt:new Date().toISOString(),layouts:w.layouts.map(n=>n.id===t.id?{...n,...e}:n)})}function it(e,t){let n=e.fields.find(e=>!e.hidden)??e.fields[0],r=n?X(e,t,n.id):``;return String(r||`Untitled record`)}function at(e,t,n,r){let i=e.fields.find(e=>e.id===n),a=X(e,t,n),o=`aria-label="${V(i?.name??`Field`)}, record ${r+1}" data-record-id="${t.id}" data-field-id="${n}"`;if(i?.type===`checkbox`)return`<input class="cell-checkbox" type="checkbox" ${o} ${a===!0||a===`true`?`checked`:``}>`;if(i?.type===`image`){let e=String(a??``);return`
        ...
        <tbody>
          ${t.map((t,n)=>`
                <tr class="${t.id===E?`active-row`:``}" data-record-row="${t.id}">
                  ...
                  ${Q(e).map(r=>`<td>${at(e,t,r.id,n)}</td>`).join(``)}
  ...
  `}function ct(e){let t=e.records.find(e=>e.id===E)??e.records[0];if(!t)return`<div class="empty-panel">Add a record to use form view.</div>`;let n=w.schema.relationships.filter(t=>t.fromTableId===e.id).map(n=>{let r=w.schema.tables.find(e=>e.id===n.toTableId),i=r?h(n,e,t,r):[];return`
  ...
  `}function lt(e,t){let n=(t,n)=>{let r=X(e,n,t.id);if(t.type===`image`){let e=String(r??``);return`
  ...
  `}function dt(e,t){let n=pe(e);return`
  ...
  `}function ft(e){let t=W(e),n={table:`Table: spreadsheet-like rows and columns for fast data entry.`,form:`Form: focus on one record at a time.`,cards:`Cards: compact text-first record cards for browsing.`,gallery:`Gallery: image-first cards for collections and exhibits.`,labels:`Labels: printable small cards or shelf labels.`,report:`Report: printable table with title and summaries.`},r=D===`form`?ct(e):D===`cards`||D===`gallery`?lt(e,t):D===`labels`?ut(e,t):D===`report`?dt(e,t):st(e,t);return`
  ...
  `}function pt(e){let t=pe(e),n=e.records.find(e=>e.id===E)??e.records[0],r=n?w.schema.relationships.filter(t=>t.fromTableId===e.id).map(t=>{let r=w.schema.tables.find(e=>e.id===t.toTableId),i=r?h(t,e,n,r).length:0;return`
  ...
  `}function mt(e){if(P===`none`)return``;if(P===`replace`){let t=L.slice(0,8);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Replace values">
          ...
          ${L.length?`<div class="replace-preview"><strong>${L.length} change${L.length===1?``:`s`} ready</strong>${t.map(t=>{let n=e.records.find(e=>e.id===t.recordId),r=e.fields.find(e=>e.id===t.fieldId);return`<p><span>${V(n?it(e,n):`Record`)} / ${V(r?.name??`Field`)}</span><del>${V(t.before)}</del><ins>${V(t.after)}</ins></p>`}).join(``)}</div>`:``}
    ...
    `}if(P===`field`){let t=e.fields.find(e=>e.id===Me)??e.fields[0];return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Field settings">
          ...
          <label>Type <select data-field-type>${Xe(t.type)}</select></label>
    ...
    `}if(P===`quality`){let t=et(e),n=$e(e);return`
      <div class="modal-backdrop">
    ...
    `}return P===`teacherNotes`?`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Teacher notes">
          ...
          <label>Notes <textarea data-teacher-notes rows="8" placeholder="Add setup directions, reflection questions, or grading reminders.">${V(w.teacher.notes.join(`
`))}</textarea></label>
          <div class="modal-actions">
  ...
  `}function $(){let e=U();G(e),Re(),C.innerHTML=`
    ...
    <main class="listsplat-app">
      ...
      <section class="toolbar" aria-label="Database toolbar">
        ...
        <label>${V(H(`Type`))} <select data-new-field-type>${Xe()}</select></label>
    ...
    ${mt(e)}
  `}function ht(e){return e instanceof HTMLInputElement&&e.type===`checkbox`?e.checked:e instanceof HTMLInputElement&&e.type===`number`?e.value===``?``:Number(e.value):e.value}function gt(e){let t=e.dataset.recordId,n=e.dataset.fieldId;!t||!n||(w=u(w,l(U(),t,n,ht(e))),p(w),O=`Saved locally`)}function _t(e,t,n,r){if(!n.type.startsWith(`image/`)){F=`That clipboard item is not an image.`,$();return}let i=new FileReader;i.addEventListener(`load`,()=>{q(r),w=u(w,l(U(),e,t,String(i.result??``))),p(w),F=`Image saved in this field.`,$()}),i.readAsDataURL(n)}function vt(){let e=U(),t=e.fields.find(e=>e.id===Me);if(!t)return;let n=C.querySelector(`[data-field-name]`)?.value??t.name,r=C.querySelector(`[data-field-type]`)?.value??t.type,i=C.querySelector(`[data-field-description]`)?.value??``,a=C.querySelector(`[data-field-required]`)?.checked??!1,o=C.querySelector(`[data-field-hidden]`)?.checked??!1,s=C.querySelector(`[data-field-formula]`)?.value??``,ee=(C.querySelector(`[data-field-options]`)?.value??``).split(`,`).map(e=>e.trim()).filter(Boolean);J(c(e,t.id,{name:n,type:r,description:i,required:a,hidden:o,formula:s,options:ee})),P=`none`,F=`Updated ${n}.`,$()}function yt(){let e=C.querySelector(`[data-replace-find]`)?.value??``,t=C.querySelector(`[data-replace-with]`)?.value??``,n=C.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=C.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=C.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=k?W(U()).map(e=>e.id):void 0;q(`replace`);let o=Te(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i});P=`none`,L=[],F=`Replaced ${o.count} value${o.count===1?``:`s`}.`,J(o.table)}function bt(){let e=C.querySelector(`[data-replace-find]`)?.value??``,t=C.querySelector(`[data-replace-with]`)?.value??``,n=C.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=C.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=C.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=k?W(U()).map(e=>e.id):void 0;L=we(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i}),F=`Preview found ${L.length} change${L.length===1?``:`s`}.`,$()}function xt(){let e=(C.querySelector(`[data-teacher-notes]`)?.value??``).split(`
`).map(e=>e.trim()).filter(Boolean);q(`teacher notes`),P=`none`,F=`Saved ${e.length} teacher note${e.length===1?``:`s`}.`,K({...w,updatedAt:new Date().toISOString(),teacher:{...w.teacher,notes:e}})}function St(e){let t=C.querySelector(e)?.value;if(!t)return null;let[n,r]=t.split(`:`);return n&&r?{tableId:n,fieldId:r}:null}function Ct(){let e=C.querySelector(`[data-relationship-from-table]`)?.value??``,t=C.querySelector(`[data-relationship-to-table]`)?.value??``,n=St(`[data-relationship-from-field]`),r=St(`[data-relationship-to-field]`),i=C.querySelector(`[data-relationship-name]`)?.value??``;if(!e||!t||!n||!r){F=`Choose both tables and both match fields.`,$();return}if(n.tableId!==e||r.tableId!==t){F=`Match fields must belong to the tables you chose.`,$();return}q(`relationship create`);let a=le(i,e,n.fieldId,t,r.fieldId);F=`Created relationship: ${a.name}.`,K(ue(w,a))}function wt(){Ne=C.querySelector(`[data-relationship-from-table]`)?.value??Ne,R=C.querySelector(`[data-relationship-to-table]`)?.value??R,$()}C.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action]`)?.dataset.action,r=t.closest(`[data-table-id]`)?.dataset.tableId,i=t.closest(`[data-template-id]`)?.dataset.templateId,c=t.closest(`[data-view-mode]`)?.dataset.viewMode,l=t.closest(`[data-select-record]`)?.dataset.selectRecord,u=t.closest(`[data-field-settings]`)?.dataset.fieldSettings,d=t.closest(`[data-record-action-id]`)?.dataset.recordActionId,f=t.closest(`[data-quality-field-id]`);if(r){T=r,N=new Set,G(U()),$();return}if(i){Ye(i);return}if(c){D=c,$();return}if(l){E=l,D===`table`&&$();return}if(u){Me=u,P=`field`,$();return}if(f){let e=f.dataset.qualityFieldId,t=f.dataset.qualityKind;if(e&&t){let n=t===`duplicates`?Ce(U(),e):x(U(),e);N=new Set(n.map(e=>e.id));let r=U().fields.find(t=>t.id===e);F=`Highlighted ${n.length} ${t===`duplicates`?`duplicate`:`missing`} record${n.length===1?``:`s`} in ${r?.name??`this field`}.`,P=`none`,$();return}}if(n)if(Be(),n===`new`){q(`new database`);let e=a(`Untitled Database`);T=e.schema.tables[0].id,E=e.schema.tables[0].records[0]?.id??``,N=new Set,K(e)}else if(n===`save-json`)He();else if(n===`open-json`)C.querySelector(`[data-open-json]`)?.click();else if(n===`import-csv`)C.querySelector(`[data-import-csv]`)?.click();else if(n===`export-csv`)Ue();else if(n===`export-report`)We();else if(n===`project-packet`)Ge();else if(n===`print`)window.print();else if(n===`add-record`)q(`add record`),J(s(U()));else if(n===`add-field`){let e=C.querySelector(`[data-new-field]`),t=C.querySelector(`[data-new-field-type]`)?.value;q(`add field`),J(o(U(),e?.value||`New Field`,t??`text`))}else if(n===`add-table`){let e=window.prompt(`New table name?`,`New Table`)??``;q(`add table`);let t=ne(w,e);T=t.schema.tables.at(-1)?.id??T,E=U().records[0]?.id??``,K(t)}else if(n===`duplicate-record`&&d)q(`duplicate record`),J(ee(U(),d));else if(n===`delete-record`&&d)q(`delete record`),J(te(U(),d));else if(n===`toggle-sort`)M=M===`asc`?`desc`:`asc`,$();else if(n===`sort`)j&&J(Se(U(),j,M));else if(n===`duplicates`){let e=A===`all`?U().fields[0]?.id:A;N=new Set(Ce(U(),e).map(e=>e.id)),F=`Found ${N.size} duplicate record${N.size===1?``:`s`}.`,$()}else if(n===`missing`){let e=A===`all`?U().fields[0]?.id:A;N=new Set(x(U(),e).map(e=>e.id)),F=`Found ${N.size} record${N.size===1?``:`s`} with missing values.`,$()}else if(n===`clear-find`)k=``,N=new Set,F=`Showing all records.`,$();else if(n===`replace`)L=[],P=`replace`,$();else if(n===`preview-replace`)bt();else if(n===`run-replace`)yt();else if(n===`save-teacher-notes`)xt();else if(n===`apply-csv-new`)Je(`new`);else if(n===`apply-csv-append`)Je(`append`);else if(n===`save-field-settings`)q(`field settings`),vt();else if(n===`layout-designer`||n===`lock-layout`)P=`layout`,$();else if(n===`layout-field-up`||n===`layout-field-down`){let e=t.closest(`[data-layout-field-id]`)?.dataset.layoutFieldId,r=Z();if(e&&r){let t=Q(U()).map(e=>e.id),r=t.indexOf(e),i=n===`layout-field-up`?r-1:r+1;r>=0&&i>=0&&i<t.length&&([t[r],t[i]]=[t[i],t[r]],q(`layout order`),rt({fieldOrder:t}),P=`layout`)}}else if(n===`save-layout-settings`){let e=C.querySelector(`[data-layout-locked]`)?.checked??!1;q(`layout settings`),rt({locked:e,fieldOrder:Q(U()).map(e=>e.id)}),P=`none`}else n===`create-relationship`?Ct():n===`undo-change`?ze():n===`close-dialog`?(P=`none`,L=[],z=null,$()):n.endsWith(`-view`)?(D=n.replace(`-view`,``),$()):n===`templates`?(F=`Template starters are in the Teacher panel.`,$()):n===`project-ideas`?(P=`projectIdeas`,$()):n===`relationships`?(P=`relationship`,$()):n===`functions`?(P=`functions`,$()):n===`quality`?(P=`quality`,$()):n===`teacher-notes`?(P=`teacherNotes`,$()):n.startsWith(`help-`)?(P=`help`,$()):(F=`This ListSplatTM tool is planned for a later build.`,$())}),C.addEventListener(`change`,e=>{let t=e.target;if(t.matches(`#languageSwitcher`)&&t instanceof HTMLSelectElement){B=Fe(t.value);try{localStorage.setItem(Oe,B)}catch{}$()}else if(t.matches(`[data-open-json]`)&&t instanceof HTMLInputElement&&t.files?.[0])Ke(t.files[0]);else if(t.matches(`[data-import-csv]`)&&t instanceof HTMLInputElement&&t.files?.[0])qe(t.files[0]);else if(t.matches(`[data-search-field]`))A=t.value,N=new Set,$();else if(t.matches(`[data-sort-field]`))j=t.value,j&&J(Se(U(),j,M));else if(t.matches(`[data-relationship-from-table], [data-relationship-to-table]`))wt();else if(t.matches(`.cell-input, .cell-checkbox, .image-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement))if(t instanceof HTMLInputElement&&t.type===`file`&&t.files?.[0]){let e=t.dataset.recordId,n=t.dataset.fieldId,r=t.files[0];e&&n&&_t(e,n,r,`image upload`)}else gt(t)}),C.addEventListener(`paste`,e=>{let t=e.target.closest(`.image-cell`);if(!t)return;let n=t.dataset.recordId,r=t.dataset.fieldId,i=Array.from(e.clipboardData?.items??[]).find(e=>e.type.startsWith(`image/`))?.getAsFile();n&&r&&i&&(e.preventDefault(),_t(n,r,i,`image paste`))}),C.addEventListener(`input`,e=>{let t=e.target;if(t.matches(`[data-project-title]`)){Ve(t.value);return}if(t.matches(`[data-search]`)){k=t.value,N=new Set,$();return}t.matches(`.cell-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement)&&gt(t)}),document.addEventListener(`click`,e=>{e.target.closest(`.menu`)||Be()}),document.addEventListener(`toggle`,e=>{let t=e.target;!(t instanceof HTMLDetailsElement)||!t.matches(`.menu`)||!t.open||document.querySelectorAll(`.menu[open]`).forEach(e=>{e!==t&&(e.open=!1)})},!0),$();