...
`)}var oe=`listsplat.autosave.v1`;function p(e){localStorage.setItem(oe,JSON.stringify(e))}function se(){let e=localStorage.getItem(oe);if(!e)return null;let t=JSON.parse(e);return d(t),t}function m(e,t,n){let r=new Blob([t],{type:n}),i=URL.createObjectURL(r),a=document.createElement(`a`);a.href=i,a.download=e,a.click(),URL.revokeObjectURL(i)}function ce(e,t){return e.records.map(e=>Number(e.values[t])).filter(e=>Number.isFinite(e))}function le(e){return e.fields.filter(e=>[`number`,`currency`,`percent`].includes(e.type)).map(t=>{let n=ce(e,t.id),r=n.reduce((e,t)=>e+t,0);return{fieldId:t.id,fieldName:t.name,count:n.length,sum:r,average:n.length?r/n.length:0,minimum:n.length?Math.min(...n):0,maximum:n.length?Math.max(...n):0}})}function ue(e){let t=[],n=``,r=!1;for(let i=0;i<e.length;i+=1){let a=e[i],o=e[i-1];if(a===`"`&&o!==`\\`){r=!r,n+=a;continue}if(a===`,`&&!r){t.push(n.trim()),n=``;continue}n+=a}return(n.trim()||e.endsWith(`,`))&&t.push(n.trim()),t}function de(e){let t=e.trim();if(t.startsWith(`"`)&&t.endsWith(`"`))return t.slice(1,-1).replace(/\\"/g,`"`)}function fe(e,t,n){let r=n.trim().toLowerCase(),i=e.fields.find(e=>e.name.toLowerCase()===r);return i?String(t.values[i.id]??``):``}function h(e,t,n){return de(n)??fe(e,t,n)}function g(e,t,n){let r=Number(h(e,t,n));return Number.isFinite(r)?r:0}function pe(e,t){let n=t.trim().toLowerCase(),r=e.fields.find(e=>e.name.toLowerCase()===n);return r?ce(e,r.id):[]}function _(e){return Number.isFinite(e)?String(Number(e.toFixed(8))):`Formula error: not a number`}function me(e){return e.toLowerCase().replace(/\b[a-z]/g,e=>e.toUpperCase())}function he(e,t,n){let r=e.trim();if(!r)return``;let i=r.match(/^([A-Z_]+)\((.*)\)$/i);if(!i)return`Formula error: use a function like FIELD(Name)`;let a=i[1].toUpperCase(),o=ue(i[2]),s=o[0]??``;if(a===`FIELD`)return fe(t,n,s);if(a===`JOIN`)return o.map(e=>h(t,n,e)).join(``);if(a===`UPPER`)return h(t,n,s).toUpperCase();if(a===`LOWER`)return h(t,n,s).toLowerCase();if(a===`TITLECASE`)return me(h(t,n,s));if(a===`TRIM`)return h(t,n,s).trim();if(a===`LENGTH`)return String(h(t,n,s).length);if(a===`CONTAINS`)return h(t,n,s).toLowerCase().includes(h(t,n,o[1]??``).toLowerCase())?`Yes`:`No`;if(a===`IF_EMPTY`)return h(t,n,s).trim()?h(t,n,s):h(t,n,o[1]??``);if(a===`ADD`)return _(o.reduce((e,r)=>e+g(t,n,r),0));if(a===`SUBTRACT`)return _(o.slice(1).reduce((e,r)=>e-g(t,n,r),g(t,n,s)));if(a===`MULTIPLY`)return _(o.reduce((e,r)=>e*g(t,n,r),1));if(a===`DIVIDE`){let e=g(t,n,o[1]??``);return e===0?`Formula error: divide by zero`:_(g(t,n,s)/e)}if(a===`ROUND`){let e=Math.max(0,Math.min(6,Math.round(g(t,n,o[1]??`"0"`))));return String(Number(g(t,n,s).toFixed(e)))}if([`SUM`,`AVERAGE`,`MIN`,`MAX`,`COUNT`].includes(a)){let e=pe(t,s);return a===`SUM`?_(e.reduce((e,t)=>e+t,0)):a===`AVERAGE`?_(e.length?e.reduce((e,t)=>e+t,0)/e.length:0):a===`MIN`?_(e.length?Math.min(...e):0):a===`MAX`?_(e.length?Math.max(...e):0):String(e.length)}return`Formula error: ${a} is not supported`}function v(e){return e==null?``:String(e)}function ge(e,t){let n=t.query.trim();if(!n)return e.records;let r=n.toLowerCase();return e.records.filter(n=>(t.fieldId===`all`?e.fields.map(e=>e.id):[t.fieldId]).some(e=>v(n.values[e]).toLowerCase().includes(r)))}function _e(e,t,n){let r=n===`asc`?1:-1;return{...e,records:[...e.records].sort((e,n)=>{let i=v(e.values[t]),a=v(n.values[t]),o=Number(i),s=Number(a);return!Number.isNaN(o)&&!Number.isNaN(s)?(o-s)*r:i.localeCompare(a,void 0,{numeric:!0,sensitivity:`base`})*r})}}function y(e,t){let n=new Map;return e.records.forEach(e=>{let r=v(e.values[t]).trim().toLowerCase();r&&n.set(r,(n.get(r)??0)+1)}),e.records.filter(e=>{let r=v(e.values[t]).trim().toLowerCase();return r?(n.get(r)??0)>1:!1})}function b(e,t){return e.records.filter(e=>!v(e.values[t]).trim())}function ve(e,t){if(!t.find)return[];let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=t.wholeWord?`\\b${r}\\b`:r,a=new RegExp(i,n),o=new Set(t.recordIds??e.records.map(e=>e.id)),s=[];return e.records.forEach(e=>{o.has(e.id)&&t.fieldIds.forEach(n=>{let r=v(e.values[n]),i=r.replace(a,t.replacement);i!==r&&s.push({recordId:e.id,fieldId:n,before:r,after:i})})}),s}function ye(e,t){if(!t.find)return{table:e,count:0};let n=t.caseSensitive?`g`:`gi`,r=t.find.replace(/[.*+?^${}()|[\]\\]/g,`\\$&`),i=t.wholeWord?`\\b${r}\\b`:r,a=new RegExp(i,n),o=new Set(t.recordIds??e.records.map(e=>e.id)),s=0,c=e.records.map(e=>{if(!o.has(e.id))return e;let n=!1,r={...e.values};return t.fieldIds.forEach(e=>{let i=v(r[e]),o=i.replace(a,()=>(s+=1,t.replacement));o!==i&&(n=!0,r[e]=o)}),n?{...e,updatedAt:new Date().toISOString(),values:r}:e});return{table:{...e,records:c},count:s}}function be(e){return e==null?``:String(e).trim().toLowerCase()}function xe(e,n,r,i,a){return{id:t(`relationship`),name:e.trim()||`New Relationship`,fromTableId:n,fromFieldId:r,toTableId:i,toFieldId:a}}function Se(e,t){return{...e,updatedAt:new Date().toISOString(),schema:{...e.schema,relationships:[...e.schema.relationships,t]}}}function Ce(e,t,n,r){let i=be(n.values[e.fromFieldId]);return!i||t.id!==e.fromTableId||r.id!==e.toTableId?[]:r.records.filter(t=>be(t.values[e.toFieldId])===i)}function we(e,t){let n=e.schema.tables.find(e=>e.id===t.fromTableId),r=e.schema.tables.find(e=>e.id===t.toTableId),i=n?.fields.find(e=>e.id===t.fromFieldId),a=r?.fields.find(e=>e.id===t.toFieldId);return`${n?.name??`Table`}:${i?.name??`Field`} -> ${r?.name??`Table`}:${a?.name??`Field`}`}function x(e,t,n,r,i,a){return{id:e,title:t,gradeBand:n,goal:r,table:re(t,i),reflectionQuestions:a}}var Te=[x(`classroom-library`,`Classroom Library Catalog`,`Grades 3-8`,`Track books, genres, reading levels, and recommendations.`,`Title,Author,Genre,Pages,Who would enjoy it?
...
Apollo 11 mission,NASA,1969,Space exploration,First humans walked on the Moon`,[`How should the exhibit be grouped?`,`What field would help visitors understand the item?`])];function Ee(e){let t=e.table.fields.map(e=>({...e,id:`${e.id}_${Date.now().toString(36)}`})),n=e.table.records.map(n=>r(t,Object.fromEntries(e.table.fields.map((e,r)=>[t[r].id,n.values[e.id]??``]))));return{...e.table,id:`table_${e.id}_${Date.now().toString(36)}`,fields:t,records:n}}var De=`drawsplat.language`,S=[{code:`en`,label:`English`,dir:`ltr`,htmlLang:`en`},{code:`es`,label:`Español`,dir:`ltr`,htmlLang:`es`},{code:`vi`,label:`Tiếng Việt`,dir:`ltr`,htmlLang:`vi`},{code:`ar`,label:`العربية`,dir:`rtl`,htmlLang:`ar`},{code:`zh`,label:`中文`,dir:`ltr`,htmlLang:`zh`},{code:`uh`,label:`हिन्दी / اردو`,dir:`ltr`,htmlLang:`hi`}],Oe={es:{New:`Nuevo`,"Save JSON":`Guardar JSON`,"Open JSON":`Abrir JSON`,"Export CSV":`Exportar CSV`,File:`Archivo`,Edit:`Editar`,Data:`Datos`,Layout:`Diseño`,Tools:`Herramientas`,View:`Vista`,Teacher:`Docente`,Help:`Ayuda`,Title:`Título`,Search:`Buscar`,In:`En`,"All fields":`Todos los campos`,Sort:`Ordenar`,"Choose field":`Elegir campo`,"New field":`Campo nuevo`,"Field name":`Nombre del campo`,Type:`Tipo`,"Add field":`Agregar campo`,"Add record":`Agregar registro`,Table:`Tabla`,Form:`Formulario`,Cards:`Tarjetas`,Gallery:`Galería`,Labels:`Etiquetas`,Report:`Informe`,"Upload image":`Subir imagen`,"No image yet":`Sin imagen todavía`},vi:{New:`Mới`,"Save JSON":`Lưu JSON`,"Open JSON":`Mở JSON`,"Export CSV":`Xuất CSV`,File:`Tệp`,Edit:`Sửa`,Data:`Dữ liệu`,Layout:`Bố cục`,Tools:`Công cụ`,View:`Xem`,Teacher:`Giáo viên`,Help:`Trợ giúp`,Title:`Tiêu đề`,Search:`Tìm`,Sort:`Sắp xếp`,Table:`Bảng`,Form:`Biểu mẫu`,Cards:`Thẻ`,Gallery:`Thư viện ảnh`,Labels:`Nhãn`,Report:`Báo cáo`},ar:{New:`جديد`,"Save JSON":`حفظ JSON`,"Open JSON":`فتح JSON`,"Export CSV":`تصدير CSV`,File:`ملف`,Edit:`تحرير`,Data:`بيانات`,Layout:`تخطيط`,Tools:`أدوات`,View:`عرض`,Teacher:`المعلم`,Help:`مساعدة`,Title:`العنوان`,Search:`بحث`,Sort:`فرز`,Table:`جدول`,Form:`نموذج`,Cards:`بطاقات`,Gallery:`معرض`,Labels:`ملصقات`,Report:`تقرير`},zh:{New:`新建`,"Save JSON":`保存 JSON`,"Open JSON":`打开 JSON`,"Export CSV":`导出 CSV`,File:`文件`,Edit:`编辑`,Data:`数据`,Layout:`布局`,Tools:`工具`,View:`视图`,Teacher:`教师`,Help:`帮助`,Title:`标题`,Search:`搜索`,Sort:`排序`,Table:`表格`,Form:`表单`,Cards:`卡片`,Gallery:`图库`,Labels:`标签`,Report:`报告`},uh:{New:`नया`,File:`फ़ाइल`,Edit:`संपादन`,Data:`डेटा`,Layout:`लेआउट`,Tools:`औज़ार`,View:`दृश्य`,Teacher:`शिक्षक`,Help:`सहायता`,Search:`खोज`,Table:`तालिका`,Form:`फ़ॉर्म`,Cards:`कार्ड`,Gallery:`गैलरी`,Labels:`लेबल`,Report:`रिपोर्ट`}},ke=document.querySelector(`#app`);if(!ke)throw Error(`ListSplatTM app root was not found.`);var C=ke,w=se()??a(),T=w.schema.tables[0].id,E=w.schema.tables[0].records[0]?.id??``,D=`table`,O=`Saved locally`,k=``,A=`all`,j=``,M=`asc`,N=new Set,P=`none`,F=``,I=`Tip: Start with one table, then add relationships when your project needs them.`,L=[],R=[],Ae=T,je=w.schema.tables[1]?.id??T,z=null,Me=``,B=Pe();function V(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function Ne(e){let t=(e||``).toLowerCase();return t.startsWith(`es`)?`es`:t.startsWith(`vi`)?`vi`:t.startsWith(`ar`)?`ar`:t.startsWith(`zh`)?`zh`:t===`uh`||t.startsWith(`ur`)||t.startsWith(`hi`)?`uh`:`en`}function Pe(){let e=new URLSearchParams(window.location.search);try{return Ne(e.get(`lang`)||localStorage.getItem(De)||navigator.language)}catch{return Ne(e.get(`lang`)||navigator.language)}}function Fe(e){return S.map(({code:t,label:n})=>`<option value="${t}"${t===e?` selected`:``}>${n}</option>`).join(``)}function H(e){return B===`en`?e:Oe[B][e]??e}function Ie(){let e=S.find(e=>e.code===B)??S[0];document.documentElement.lang=e.htmlLang,document.documentElement.dir=e.dir}function U(){return w.schema.tables.find(e=>e.id===T)??w.schema.tables[0]}function W(e){let t=ge(e,{query:k,fieldId:A});return N.size>0?t.filter(e=>N.has(e.id)):t}function G(e){e.records.some(e=>e.id===E)||(E=e.records[0]?.id??``)}function K(e){w=e,G(U()),p(w),O=`Saved locally`,$()}function q(e){L=[{label:e,project:structuredClone(w)},...L].slice(0,12)}function Le(){let e=L[0];if(!e){I=`Nothing to undo yet.`,$();return}L=L.slice(1),w=e.project,T=w.schema.tables.some(e=>e.id===T)?T:w.schema.tables[0].id,G(U()),p(w),I=`Undid ${e.label}.`,$()}function J(e){T=e.id,K(u(w,e))}function Re(){document.querySelectorAll(`.menu[open]`).forEach(e=>{e.open=!1})}function ze(e){K({...w,updatedAt:new Date().toISOString(),metadata:{...w.metadata,title:e||`Untitled Database`}})}function Be(){m(`${w.metadata.title||`listsplat-project`}.listsplat.json`,JSON.stringify(w,null,2),`application/json`)}function Ve(){m(`${U().name}.csv`,ae(U()),`text/csv;charset=utf-8`)}function He(){let e=U(),t=W(e),n=`<!doctype html>
<html lang="en">
...
<tbody>${t.map(t=>`<tr>${e.fields.map(n=>`<td>${V(X(e,t,n.id))}</td>`).join(``)}</tr>`).join(``)}</tbody></table></body></html>`;m(`${w.metadata.title||`listsplat-report`}.html`,n,`text/html;charset=utf-8`)}function Ue(e){e.text().then(e=>{let t=JSON.parse(e);d(t),T=t.schema.tables[0].id,E=t.schema.tables[0].records[0]?.id??``,K(t)}).catch(e=>{window.alert(e instanceof Error?e.message:`Could not open this ListSplatTM file.`)})}function We(e){e.text().then(t=>{let n=re(e.name.replace(/\.csv$/i,``),t);z=n,Me=e.name,P=`csvImport`,I=`Previewing ${n.records.length} CSV record${n.records.length===1?``:`s`} from ${e.name}.`,$()})}function Ge(e){if(!z){P=`none`;return}if(q(`CSV import`),e===`new`){let e=z;T=e.id,E=e.records[0]?.id??``,z=null,P=`none`,I=`Imported ${e.records.length} records from ${Me}.`,K({...w,updatedAt:new Date().toISOString(),schema:{...w.schema,tables:[...w.schema.tables,e]},layouts:[...w.layouts,{id:`layout_${Date.now().toString(36)}`,name:`${e.name} Table`,tableId:e.id,mode:`table`,locked:!1}]});return}let t=U(),n=new Map(z.fields.map(e=>[e.name.trim().toLowerCase(),e.id])),i=z.records.map(e=>r(t.fields,Object.fromEntries(t.fields.map(t=>{let r=n.get(t.name.trim().toLowerCase());return[t.id,r?e.values[r]??``:``]}))));z=null,P=`none`,I=`Appended ${i.length} CSV record${i.length===1?``:`s`} to ${t.name}.`,J({...t,records:[...t.records,...i]})}function Ke(e){let t=Te.find(t=>t.id===e);if(!t)return;let n=Ee(t);q(`template load`),T=n.id,E=n.records[0]?.id??``,I=`Loaded ${t.title}.`,K({...w,metadata:{...w.metadata,title:t.title},schema:{...w.schema,tables:[...w.schema.tables,n]},teacher:{...w.teacher,notes:t.reflectionQuestions}})}function Y(e,t){return`
    <details class="menu">
  ...
  `}function qe(e=`text`){return[[`text`,`Short text`],[`longText`,`Long text`],[`number`,`Number`],[`currency`,`Currency`],[`percent`,`Percent`],[`date`,`Date`],[`checkbox`,`Checkbox`],[`rating`,`Rating`],[`choice`,`Choice`],[`image`,`Image`],[`link`,`Link`],[`calculation`,`Calculation`],[`autoNumber`,`Auto number`],[`createdAt`,`Created time`],[`updatedAt`,`Updated time`]].map(([t,n])=>`<option value="${t}" ${e===t?`selected`:``}>${n}</option>`).join(``)}function X(e,t,n){let r=e.fields.find(e=>e.id===n);return r?.type===`calculation`&&r.formula?he(r.formula,e,t):t.values[n]??``}function Z(){return w.layouts.find(e=>e.tableId===T&&e.mode===D)}function Je(e){return!!e}function Q(e){let t=Z()?.fieldOrder??e.fields.map(e=>e.id),n=new Map(e.fields.map(e=>[e.id,e]));return[...t.map(e=>n.get(e)).filter(Je),...e.fields.filter(e=>!t.includes(e.id))].filter(e=>e&&!e.hidden)}function Ye(e){return e.fields.filter(e=>e.type===`calculation`)}function Xe(e){return Ye(e).reduce((t,n)=>n.formula?t+e.records.filter(t=>String(he(n.formula??``,e,t)).startsWith(`Formula error:`)).length:t,0)}function Ze(e){return e.fields.filter(e=>!e.hidden&&![`image`,`autoNumber`,`createdAt`,`updatedAt`,`calculation`].includes(e.type)).map(t=>({field:t,missing:b(e,t.id).length,duplicates:y(e,t.id).length}))}function Qe(e){return Q(e).filter(e=>e.type===`image`)}function $e(e,t){let n=Qe(e)[0];return n?String(X(e,t,n.id)??``):``}function et(e){let t=Z();t&&K({...w,updatedAt:new Date().toISOString(),layouts:w.layouts.map(n=>n.id===t.id?{...n,...e}:n)})}function tt(e,t){let n=e.fields.find(e=>!e.hidden)??e.fields[0],r=n?X(e,t,n.id):``;return String(r||`Untitled record`)}function nt(e,t,n,r){let i=e.fields.find(e=>e.id===n),a=X(e,t,n),o=`aria-label="${V(i?.name??`Field`)}, record ${r+1}" data-record-id="${t.id}" data-field-id="${n}"`;if(i?.type===`checkbox`)return`<input class="cell-checkbox" type="checkbox" ${o} ${a===!0||a===`true`?`checked`:``}>`;if(i?.type===`image`){let e=String(a??``);return`
        ...
        <tbody>
          ${t.map((t,n)=>`
                <tr class="${t.id===E?`active-row`:``}" data-record-row="${t.id}">
                  ...
                  ${Q(e).map(r=>`<td>${nt(e,t,r.id,n)}</td>`).join(``)}
  ...
  `}function at(e){let t=e.records.find(e=>e.id===E)??e.records[0];if(!t)return`<div class="empty-panel">Add a record to use form view.</div>`;let n=w.schema.relationships.filter(t=>t.fromTableId===e.id).map(n=>{let r=w.schema.tables.find(e=>e.id===n.toTableId),i=r?Ce(n,e,t,r):[];return`
  ...
  `}function ot(e,t){let n=(t,n)=>{let r=X(e,n,t.id);if(t.type===`image`){let e=String(r??``);return`
  ...
  `}function ct(e,t){let n=le(e);return`
  ...
  `}function lt(e){let t=W(e),n={table:`Table: spreadsheet-like rows and columns for fast data entry.`,form:`Form: focus on one record at a time.`,cards:`Cards: compact text-first record cards for browsing.`,gallery:`Gallery: image-first cards for collections and exhibits.`,labels:`Labels: printable small cards or shelf labels.`,report:`Report: printable table with title and summaries.`},r=D===`form`?at(e):D===`cards`||D===`gallery`?ot(e,t):D===`labels`?st(e,t):D===`report`?ct(e,t):it(e,t);return`
  ...
  `}function ut(e){let t=le(e),n=e.records.find(e=>e.id===E)??e.records[0],r=n?w.schema.relationships.filter(t=>t.fromTableId===e.id).map(t=>{let r=w.schema.tables.find(e=>e.id===t.toTableId),i=r?Ce(t,e,n,r).length:0;return`
  ...
  `}function dt(e){if(P===`none`)return``;if(P===`replace`){let t=R.slice(0,8);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Replace values">
          ...
          ${R.length?`<div class="replace-preview"><strong>${R.length} change${R.length===1?``:`s`} ready</strong>${t.map(t=>{let n=e.records.find(e=>e.id===t.recordId),r=e.fields.find(e=>e.id===t.fieldId);return`<p><span>${V(n?tt(e,n):`Record`)} / ${V(r?.name??`Field`)}</span><del>${V(t.before)}</del><ins>${V(t.after)}</ins></p>`}).join(``)}</div>`:``}
    ...
    `}if(P===`field`){let t=e.fields.find(e=>e.id===F)??e.fields[0];return`
      <div class="modal-backdrop">
    ...
    `}if(P===`csvImport`&&z){let t=z.records.slice(0,5);return`
      <div class="modal-backdrop">
        <section class="modal wide-modal" role="dialog" aria-modal="true" aria-label="CSV import preview">
          ...
          <p>${V(Me)} has ${z.fields.length} field${z.fields.length===1?``:`s`} and ${z.records.length} record${z.records.length===1?``:`s`}.</p>
    ...
    `;if(P===`relationship`){let t=w.schema.tables.find(e=>e.id===Ae)??e,n=w.schema.tables.find(e=>e.id===je)??e,r=t.fields.map(e=>`<option value="${t.id}:${e.id}">${V(e.name)}</option>`).join(``),i=n.fields.map(e=>`<option value="${n.id}:${e.id}">${V(e.name)}</option>`).join(``);return`
      <div class="modal-backdrop">
        <section class="modal" role="dialog" aria-modal="true" aria-label="Relationships">
          ...
          ${w.schema.relationships.length?`<div class="relationship-list">${w.schema.relationships.map(e=>`<p><strong>${V(e.name)}</strong><br>${V(we(w,e))}</p>`).join(``)}</div>`:`<p>No relationships yet. Add a second table first for the most useful results.</p>`}
          <div class="modal-actions">
    ...
    `}if(P===`quality`){let t=Ze(e),n=Xe(e);return`
      <div class="modal-backdrop">
  ...
  `}function $(){let e=U();G(e),Ie(),C.innerHTML=`
    ...
    ${dt(e)}
  `}function ft(e){return e instanceof HTMLInputElement&&e.type===`checkbox`?e.checked:e instanceof HTMLInputElement&&e.type===`number`?e.value===``?``:Number(e.value):e.value}function pt(e){let t=e.dataset.recordId,n=e.dataset.fieldId;!t||!n||(w=u(w,l(U(),t,n,ft(e))),p(w),O=`Saved locally`)}function mt(e,t,n,r){if(!n.type.startsWith(`image/`)){I=`That clipboard item is not an image.`,$();return}let i=new FileReader;i.addEventListener(`load`,()=>{q(r),w=u(w,l(U(),e,t,String(i.result??``))),p(w),I=`Image saved in this field.`,$()}),i.readAsDataURL(n)}function ht(){let e=U(),t=e.fields.find(e=>e.id===F);if(!t)return;let n=C.querySelector(`[data-field-name]`)?.value??t.name,r=C.querySelector(`[data-field-type]`)?.value??t.type,i=C.querySelector(`[data-field-description]`)?.value??``,a=C.querySelector(`[data-field-required]`)?.checked??!1,o=C.querySelector(`[data-field-hidden]`)?.checked??!1,s=C.querySelector(`[data-field-formula]`)?.value??``,ee=(C.querySelector(`[data-field-options]`)?.value??``).split(`,`).map(e=>e.trim()).filter(Boolean);J(c(e,t.id,{name:n,type:r,description:i,required:a,hidden:o,formula:s,options:ee})),P=`none`,I=`Updated ${n}.`,$()}function gt(){let e=C.querySelector(`[data-replace-find]`)?.value??``,t=C.querySelector(`[data-replace-with]`)?.value??``,n=C.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=C.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=C.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=k?W(U()).map(e=>e.id):void 0;q(`replace`);let o=ye(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i});P=`none`,R=[],I=`Replaced ${o.count} value${o.count===1?``:`s`}.`,J(o.table)}function _t(){let e=C.querySelector(`[data-replace-find]`)?.value??``,t=C.querySelector(`[data-replace-with]`)?.value??``,n=C.querySelector(`[data-replace-field]`)?.value??U().fields[0]?.id,r=C.querySelector(`[data-replace-case-sensitive]`)?.checked??!1,i=C.querySelector(`[data-replace-whole-word]`)?.checked??!1,a=k?W(U()).map(e=>e.id):void 0;R=ve(U(),{fieldIds:[n],find:e,replacement:t,recordIds:a,caseSensitive:r,wholeWord:i}),I=`Preview found ${R.length} change${R.length===1?``:`s`}.`,$()}function vt(e){let t=C.querySelector(e)?.value;if(!t)return null;let[n,r]=t.split(`:`);return n&&r?{tableId:n,fieldId:r}:null}function yt(){let e=C.querySelector(`[data-relationship-from-table]`)?.value??``,t=C.querySelector(`[data-relationship-to-table]`)?.value??``,n=vt(`[data-relationship-from-field]`),r=vt(`[data-relationship-to-field]`),i=C.querySelector(`[data-relationship-name]`)?.value??``;if(!e||!t||!n||!r){I=`Choose both tables and both match fields.`,$();return}if(n.tableId!==e||r.tableId!==t){I=`Match fields must belong to the tables you chose.`,$();return}q(`relationship create`);let a=xe(i,e,n.fieldId,t,r.fieldId);I=`Created relationship: ${a.name}.`,K(Se(w,a))}function bt(){Ae=C.querySelector(`[data-relationship-from-table]`)?.value??Ae,je=C.querySelector(`[data-relationship-to-table]`)?.value??je,$()}C.addEventListener(`click`,e=>{let t=e.target,n=t.closest(`[data-action]`)?.dataset.action,r=t.closest(`[data-table-id]`)?.dataset.tableId,i=t.closest(`[data-template-id]`)?.dataset.templateId,c=t.closest(`[data-view-mode]`)?.dataset.viewMode,l=t.closest(`[data-select-record]`)?.dataset.selectRecord,u=t.closest(`[data-field-settings]`)?.dataset.fieldSettings,d=t.closest(`[data-record-action-id]`)?.dataset.recordActionId,f=t.closest(`[data-quality-field-id]`);if(r){T=r,N=new Set,G(U()),$();return}if(i){Ke(i);return}if(c){D=c,$();return}if(l){E=l,D===`table`&&$();return}if(u){F=u,P=`field`,$();return}if(f){let e=f.dataset.qualityFieldId,t=f.dataset.qualityKind;if(e&&t){let n=t===`duplicates`?y(U(),e):b(U(),e);N=new Set(n.map(e=>e.id));let r=U().fields.find(t=>t.id===e);I=`Highlighted ${n.length} ${t===`duplicates`?`duplicate`:`missing`} record${n.length===1?``:`s`} in ${r?.name??`this field`}.`,P=`none`,$();return}}if(n)if(Re(),n===`new`){q(`new database`);let e=a(`Untitled Database`);T=e.schema.tables[0].id,E=e.schema.tables[0].records[0]?.id??``,N=new Set,K(e)}else if(n===`save-json`)Be();else if(n===`open-json`)C.querySelector(`[data-open-json]`)?.click();else if(n===`import-csv`)C.querySelector(`[data-import-csv]`)?.click();else if(n===`export-csv`)Ve();else if(n===`export-report`||n===`project-packet`)He();else if(n===`print`)window.print();else if(n===`add-record`)q(`add record`),J(s(U()));else if(n===`add-field`){let e=C.querySelector(`[data-new-field]`),t=C.querySelector(`[data-new-field-type]`)?.value;q(`add field`),J(o(U(),e?.value||`New Field`,t??`text`))}else if(n===`add-table`){let e=window.prompt(`New table name?`,`New Table`)??``;q(`add table`);let t=ne(w,e);T=t.schema.tables.at(-1)?.id??T,E=U().records[0]?.id??``,K(t)}else if(n===`duplicate-record`&&d)q(`duplicate record`),J(ee(U(),d));else if(n===`delete-record`&&d)q(`delete record`),J(te(U(),d));else if(n===`toggle-sort`)M=M===`asc`?`desc`:`asc`,$();else if(n===`sort`)j&&J(_e(U(),j,M));else if(n===`duplicates`){let e=A===`all`?U().fields[0]?.id:A;N=new Set(y(U(),e).map(e=>e.id)),I=`Found ${N.size} duplicate record${N.size===1?``:`s`}.`,$()}else if(n===`missing`){let e=A===`all`?U().fields[0]?.id:A;N=new Set(b(U(),e).map(e=>e.id)),I=`Found ${N.size} record${N.size===1?``:`s`} with missing values.`,$()}else if(n===`clear-find`)k=``,N=new Set,I=`Showing all records.`,$();else if(n===`replace`)R=[],P=`replace`,$();else if(n===`preview-replace`)_t();else if(n===`run-replace`)gt();else if(n===`apply-csv-new`)Ge(`new`);else if(n===`apply-csv-append`)Ge(`append`);else if(n===`save-field-settings`)q(`field settings`),ht();else if(n===`layout-designer`||n===`lock-layout`)P=`layout`,$();else if(n===`layout-field-up`||n===`layout-field-down`){let e=t.closest(`[data-layout-field-id]`)?.dataset.layoutFieldId,r=Z();if(e&&r){let t=Q(U()).map(e=>e.id),r=t.indexOf(e),i=n===`layout-field-up`?r-1:r+1;r>=0&&i>=0&&i<t.length&&([t[r],t[i]]=[t[i],t[r]],q(`layout order`),et({fieldOrder:t}),P=`layout`)}}else if(n===`save-layout-settings`){let e=C.querySelector(`[data-layout-locked]`)?.checked??!1;q(`layout settings`),et({locked:e,fieldOrder:Q(U()).map(e=>e.id)}),P=`none`}else n===`create-relationship`?yt():n===`undo-change`?Le():n===`close-dialog`?(P=`none`,R=[],z=null,$()):n.endsWith(`-view`)?(D=n.replace(`-view`,``),$()):n===`templates`?(I=`Template starters are in the Teacher panel.`,$()):n===`project-ideas`?(P=`projectIdeas`,$()):n===`relationships`?(P=`relationship`,$()):n===`functions`?(P=`functions`,$()):n===`quality`?(P=`quality`,$()):n.startsWith(`help-`)?(P=`help`,$()):(I=`This ListSplatTM tool is planned for a later build.`,$())}),C.addEventListener(`change`,e=>{let t=e.target;if(t.matches(`#languageSwitcher`)&&t instanceof HTMLSelectElement){B=Ne(t.value);try{localStorage.setItem(De,B)}catch{}$()}else if(t.matches(`[data-open-json]`)&&t instanceof HTMLInputElement&&t.files?.[0])Ue(t.files[0]);else if(t.matches(`[data-import-csv]`)&&t instanceof HTMLInputElement&&t.files?.[0])We(t.files[0]);else if(t.matches(`[data-search-field]`))A=t.value,N=new Set,$();else if(t.matches(`[data-sort-field]`))j=t.value,j&&J(_e(U(),j,M));else if(t.matches(`[data-relationship-from-table], [data-relationship-to-table]`))bt();else if(t.matches(`.cell-input, .cell-checkbox, .image-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement))if(t instanceof HTMLInputElement&&t.type===`file`&&t.files?.[0]){let e=t.dataset.recordId,n=t.dataset.fieldId,r=t.files[0];e&&n&&mt(e,n,r,`image upload`)}else pt(t)}),C.addEventListener(`paste`,e=>{let t=e.target.closest(`.image-cell`);if(!t)return;let n=t.dataset.recordId,r=t.dataset.fieldId,i=Array.from(e.clipboardData?.items??[]).find(e=>e.type.startsWith(`image/`))?.getAsFile();n&&r&&i&&(e.preventDefault(),mt(n,r,i,`image paste`))}),C.addEventListener(`input`,e=>{let t=e.target;if(t.matches(`[data-project-title]`)){ze(t.value);return}if(t.matches(`[data-search]`)){k=t.value,N=new Set,$();return}t.matches(`.cell-input`)&&(t instanceof HTMLInputElement||t instanceof HTMLTextAreaElement||t instanceof HTMLSelectElement)&&pt(t)}),document.addEventListener(`click`,e=>{e.target.closest(`.menu`)||Re()}),document.addEventListener(`toggle`,e=>{let t=e.target;!(t instanceof HTMLDetailsElement)||!t.matches(`.menu`)||!t.open||document.querySelectorAll(`.menu[open]`).forEach(e=>{e!==t&&(e.open=!1)})},!0),$();