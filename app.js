...
function objectDown(e){if(tool==='eraser') return; e.stopPropagation();const o=findObj(e.currentTarget.dataset.id);if(!o)return; const p=pt(e); if(tool==='coloringpaint'){if(startColoringStroke(p)) return; setStatus('Select a coloring page, then drag inside it to paint.','danger'); return} if(board.assignmentMode&&board.mode==='student'&&o.layer==='teacher'){setStatus('Teacher-layer items are protected in assignment mode.','danger'); return} if(tool==='dotpaint'){if(o.type!=='dot') return setStatus('Dot Paint colors dot-picture dots only.','danger'); if(!canEditObject(o)) return setStatus('That dot is locked.','danger'); dotPaintDrag={active:true,moved:false,startX:p.x,startY:p.y,color:paintColor(),painted:new Set,clickDotId:o.id}; selectedIds=[o.id]; render(); return} const _now=performance.now(); if(tool==='select'&&_lastObjClick.id===o.id&&(_now-_lastObjClick.t)<500){_lastObjClick={id:null,t:0}; if(TEXTABLE_TYPES.includes(o.type)){openInlineTextEditor(o.id); return} if(o.type==='image'&&o.pictureGraphConfig){openPictureGraphDialog(o.id); return} if(o.type==='image'&&o.graphConfig){openGraphDialog(o.id); return} if(o.type==='image'&&o.mermaidSource){openMermaidDialog(o.id); return} if(o.type==='image'&&o.wordCloudSource){openWordCloudDialog(o.id); return}} _lastObjClick={id:o.id,t:_now}; if(tool==='connector'){if(o.type==='connector')return; if(!connectorPendingFrom){connectorPendingFrom=o.id; setSingleSelection(o.id); render(); setStatus('Connector: select the second shape.','success'); return}else if(connectorPendingFrom!==o.id){panel().objects.push(makeObj('connector',0,0,0,0,{fromId:connectorPendingFrom,toId:o.id,fill:'none'})); connectorPendingFrom=null; render(); saveState(); setStatus('Connector added.','success'); return}else{connectorPendingFrom=null; setStatus('Connector cancelled.'); return}}
  ...
  const dragIds=idsWithColoringPaint(selectedIds.filter(idv=>{const s=findObj(idv);return s&&s.type!=='connector'&&canEditObject(s)&&!s.locked}));
...
(function(){
  ...
  function groupToolPalette(){
    ...
    const groups={nav:['select'],draw:['pen','eraser','laser'],dot:['dotpaint'],text:['text','sticky','comment','audio']};
    Object.entries(groups).forEach(([name,items])=>items.forEach(t=>tools.querySelector(`[data-tool="${t}"]`)?.classList.add('tool-group-'+name)));
    gid('dotPictureToolBtn')?.classList.add('tool-group-dot');
    const shapeTools=['line','arrow','rect','ellipse','connector','diamond','triangle','callout','speech'];
    const first=shapeTools.map(t=>tools.querySelector(`[data-tool="${t}"]`)).find(Boolean);
    if(first){
      const details=document.createElement('details');
      details.id='shapeToolGroup';
      details.className='tool-popover-group tool-group-shapes';
      tools.insertBefore(details,first);
      const summary=document.createElement('summary');
      summary.className='tool-popover-trigger icon-btn';
      summary.setAttribute('aria-label',tr('Shapes and Lines'));
      summary.setAttribute('data-tooltip',tr('Shapes and Lines'));
      summary.innerHTML=`<span class="icon-symbol" aria-hidden="true">${icons.shapeGroup}</span><span class="icon-label">${esc(tr('Shapes and Lines'))}</span>`;
      const panelEl=document.createElement('div');
      panelEl.className='tool-popover-panel';
      shapeTools.forEach(t=>{
        const btn=tools.querySelector(`[data-tool="${t}"]`);
        if(btn){btn.classList.add('tool-group-shapes'); panelEl.appendChild(btn)}
      });
      details.append(summary,panelEl);
      panelEl.addEventListener('click',event=>{if(event.target.closest('[data-tool]')) details.open=false});
    }
  }
  function applyIcons(){
    ensureSimpleExtras?.();
