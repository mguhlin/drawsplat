...
function shell(): void {
  app.innerHTML = `<div class="spectrum"></div><main class="app">
    ...
    <section class="transport" aria-label="Transport controls">
      <button class="btn record" data-action="record">${icon('●','record')}</button><button class="btn" data-action="pause-record" disabled>${icon('❚❚','pause')}</button><button class="btn" data-action="play">${icon('▶','play')}</button><button class="btn icon" data-action="stop" title="${t('stop')}">■<span class="visually-hidden" data-i18n="stop"></span></button><button class="btn icon" data-action="start" title="${t('start')}">|◀<span class="visually-hidden" data-i18n="start"></span></button>
      <span class="transport-time" id="time">0:00.00</span><div class="meter" role="meter" aria-label="Input level" aria-valuemin="0" aria-valuemax="100"><div class="meter-fill" id="meter"></div></div>
...
function bindTrack(trackId: string): void {
  ...
  row.querySelector('[data-delete-track]')?.addEventListener('click',()=>{ if (project.tracks.length>1 && confirm(t('confirmDeleteTrack'))) commit(()=>{project.tracks=project.tracks.filter((item)=>item.id!==trackId); selectedTrackId=project.tracks[0].id;}); });
...
function commit(change:()=>void):void{history.push(cloneProject(project));if(history.length>100)history.shift();future=[];change();project.updatedAt=new Date().toISOString();renderWorkspace();scheduleSave();}
...
async function togglePlay():Promise<void>{if(playing){stopAll();return;}if(projectDuration(project)<=0)return;playing=true;document.querySelector('[data-action="play"]')?.classList.add('primary');await engine.play(project,sources,project.view.playhead,(time)=>setPlayhead(time),()=>{playing=false;updateTransport();});}
...
async function finishRecording():Promise<void>{const blob=new Blob(recordingChunks,{type:recorder?.mimeType||'audio/webm'});stopRecordingStream();if(blob.size>0)try{await addBlob(blob,`${t('recordingDefault')} ${new Date().toLocaleTimeString()}`);}catch{toast(t('importFailed'),true);}setStatus(t('ready'));}
function stopRecordingStream():void{cancelAnimationFrame(meterFrame);recordingStream?.getTracks().forEach((track)=>track.stop());recordingStream=null;recorder=null;document.querySelector('[data-action="record"]')?.classList.remove('active');const pause=document.querySelector<HTMLButtonElement>('[data-action="pause-record"]');if(pause)pause.disabled=true;const meter=document.querySelector<HTMLElement>('#meter');if(meter)meter.style.width='0';}
...
shell();
void loadWorkspace().then(async(stored)=>{if(!stored||!validateProject(stored.project))return;project=stored.project;sources=new Map(stored.sources.map((source)=>[source.id,source]));selectedTrackId=project.tracks[0]?.id??'';for(const source of sources.values()){try{await engine.loadSource(source);}catch{/* preserve metadata and let UI recover */}}renderWorkspace();}).catch(()=>setStatus(t('ready')));
