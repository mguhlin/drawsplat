...
function bindTrack(trackId: string): void {
  ...
  row.querySelector('[data-delete-track]')?.addEventListener('click',()=>{ if (confirm(t('confirmDeleteTrack'))) commit(()=>{project.tracks=project.tracks.filter((item)=>item.id!==trackId);if(project.tracks.length===0){const id=uid('track');project.tracks.push({id,name:`${t('trackDefault')} 1`,muted:false,solo:false,gain:1,pan:0,clips:[]});}selectedTrackId=project.tracks[0].id;selectedClipId=null;cleanupUnusedSources();}); });
