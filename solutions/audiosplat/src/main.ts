...
async function finishRecording():Promise<void>{const mimeType=recorder?.mimeType||recordingChunks[0]?.type||'audio/webm';const blob=new Blob(recordingChunks,{type:mimeType});stopRecordingStream();if(blob.size===0){toast(t('recordingFailed'),true);setStatus(t('ready'));return;}try{project.view.playhead=recordingInsertAt;await addBlob(blob,`${t('recordingDefault')} ${new Date().toLocaleTimeString()}`);setPlayhead(recordingInsertAt);setStatus(t('recordingReady'));}catch{toast(t('recordingFailed'),true);setStatus(t('ready'));}}
