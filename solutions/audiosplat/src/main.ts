...
async function handleAction(event: Event): Promise<void> {
  ...
  else if (action === 'record') await requestRecording('microphone'); else if (action === 'record-system') await requestRecording('system'); else if (action === 'pause-record') toggleRecordingPause();
...
function startClipDrag(event: PointerEvent, node: HTMLElement, trackId: string): void {
  ...
  const sourceTrack=track!;selectedClipId=clip.id; selectedTrackId=trackId; const startX=event.clientX; const original=clip.start; let moved=false; let destinationId=trackId; node.setPointerCapture(event.pointerId);
  const move=(next:PointerEvent)=>{const delta=(next.clientX-startX)/project.view.zoom;if(Math.abs(delta)>0.02)moved=true;clip.start=Math.max(0,original+delta);node.style.left=`${clip.start*project.view.zoom}px`;destinationId=document.elementFromPoint(next.clientX,next.clientY)?.closest<HTMLElement>('[data-track]')?.dataset.track??destinationId;};
  const up=(next:PointerEvent)=>{node.removeEventListener('pointermove',move);node.removeEventListener('pointerup',up);destinationId=document.elementFromPoint(next.clientX,next.clientY)?.closest<HTMLElement>('[data-track]')?.dataset.track??destinationId;const changedTrack=destinationId!==trackId&&project.tracks.some((item)=>item.id===destinationId);if(moved||changedTrack){const finalStart=clip.start;clip.start=original;commit(()=>{clip.start=finalStart;if(changedTrack){sourceTrack.clips=sourceTrack.clips.filter((item)=>item.id!==clip.id);project.tracks.find((item)=>item.id===destinationId)!.clips.push(clip);selectedTrackId=destinationId;}});}else renderWorkspace();};
...
async function requestRecording(kind:'microphone'|'system'='microphone'):Promise<void>{if(recorder&&recorder.state!=='inactive'){stopAll();return;}const system=kind==='system';showDialog(system?t('deviceAudio'):t('microphone'),`<p>${system?t('systemAudioIntro'):t('micIntro')}</p><div class="dialog-actions"><button class="btn" data-dialog-close data-i18n="cancel">${t('cancel')}</button><button class="btn primary" id="allow-recording">${system?t('chooseSharedAudio'):t('allowMic')}</button></div>`);document.querySelector('#allow-recording')?.addEventListener('click',()=>{closeDialog();void startRecording(kind);});}
async function startRecording(kind:'microphone'|'system'='microphone'):Promise<void>{try{if(!window.isSecureContext||!navigator.mediaDevices?.getUserMedia||typeof MediaRecorder==='undefined'){toast(t('micUnsupported'),true);return;}recordingInsertAt=project.view.playhead;if(kind==='system'){recordingStream=await acquireSystemAudio();}else{const selected=document.querySelector<HTMLSelectElement>('#mic-input')?.value;recordingStream=await acquireMicrophone(selected);await refreshMicrophones(recordingStream);}const supported=typeof MediaRecorder.isTypeSupported==='function';const mime=supported?['audio/webm;codecs=opus','audio/ogg;codecs=opus','audio/mp4','audio/webm'].find((type)=>MediaRecorder.isTypeSupported(type)):undefined;recorder=new MediaRecorder(recordingStream,mime?{mimeType:mime}:undefined);recordingChunks=[];recorder.ondataavailable=(event)=>{if(event.data.size>0)recordingChunks.push(event.data);};recorder.onerror=()=>{toast(t('recordingFailed'),true);stopRecordingStream();};recorder.onstop=()=>void finishRecording();recorder.start(250);recordingStartedAt=performance.now();startRecordingClock();startMeter(recordingStream);document.querySelector(`[data-action="${kind==='system'?'record-system':'record'}"]`)?.classList.add('active');const pause=document.querySelector<HTMLButtonElement>('[data-action="pause-record"]');if(pause)pause.disabled=false;setStatus(t('recording'));}catch(error){const denied=error instanceof DOMException&&(error.name==='NotAllowedError'||error.name==='SecurityError');toast(denied?t('permissionDenied'):kind==='system'?t('systemAudioUnavailable'):t('micUnavailable'),true);stopRecordingStream();}}
...
async function acquireSystemAudio():Promise<MediaStream>{if(!navigator.mediaDevices.getDisplayMedia)throw new DOMException('Display audio is unsupported','NotSupportedError');const display=await navigator.mediaDevices.getDisplayMedia({video:true,audio:true});const audio=display.getAudioTracks();if(audio.length===0){display.getTracks().forEach((track)=>track.stop());throw new DOMException('No shared audio track','NotFoundError');}display.getVideoTracks().forEach((track)=>track.stop());return new MediaStream(audio);}
