const CACHE="mediasplat-v11";const ROOT="/solutions/mediasplat/";
...
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("mediasplat-v")&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
