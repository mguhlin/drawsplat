...
test('transcribes using an actual GGML model selected from disk without model downloads', async ({ page }) => {
  ...
  await open(page);
  if (process.env.LOCAL_GGML_LONG) await page.locator('.drop-zone input').setInputFiles(resolve('../shared/subtitles/tests/resume.mp3'));
  ...
  if (process.env.LOCAL_GGML_LONG) {
