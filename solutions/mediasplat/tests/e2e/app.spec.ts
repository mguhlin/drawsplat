...
test("loads locally and exposes all workflows", async ({ page }) => { const external: string[] = []; page.on("request", request => { if (new URL(request.url()).hostname !== "127.0.0.1") external.push(request.url()); }); await page.goto("./"); await expect(page.getByRole("heading", { name: "Cut and combine media without the upload." })).toBeVisible(); await expect(page.getByRole("button", { name: /Local only/ })).toBeVisible(); await expect(page.getByRole("navigation", { name: "Media tools" }).getByRole("button")).toHaveCount(4); expect(external).toEqual([]); });
