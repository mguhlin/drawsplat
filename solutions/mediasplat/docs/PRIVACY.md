...
Runtime requests are limited to same-origin application assets, the service worker, `ffmpeg-core.js`, and `ffmpeg-core.wasm`. The service worker caches these files for offline reuse. Media bytes are never placed in an HTTP request body by application code.
