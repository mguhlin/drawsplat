...
Current timeline status also includes direct drag-to-select clip waveform ranges,
with range-aware cut/copy and paste-at-playhead editing. Current composition/audio
status: layered image/video preview, numeric position,
scale, rotation, opacity, per-clip fit/fill/stretch framing, per-clip gain, and
fade-in/fade-out playback are implemented. On-canvas handles, freeform crop,
panning, normalization, ducking, and detached audio remain. 4. **Titles and transitions:** SVG title/lower-third templates, SRT/WebVTT captions,
