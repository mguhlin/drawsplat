...
export function App() {
  ...
  useEffect(() => {
    for (const track of project.tracks.filter((item) => item.kind === "audio")) for (const clip of track.clips) {
      const media = timelineAudio.current.get(clip.id); if (!media) continue;
      const active = playing && !track.muted && !track.hidden && time >= clip.start && time < clip.start + clip.duration;
      if (!active) { if (!media.paused) media.pause(); continue; }
      const expected = clip.sourceStart + time - clip.start;
      if (Math.abs(media.currentTime - expected) > .4) media.currentTime = expected;
      if (media.paused) media.play().catch(() => setStatus("Browser blocked timeline audio playback — press Play again"));
    }
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => { const elapsed = (now - previous) / 1000; previous = now; setTime((current) => { const next = current + elapsed; if (next >= projectDuration(project)) { setPlaying(false); return projectDuration(project); } return next; }); frame = requestAnimationFrame(tick); };
  ...
  const previewLocation = timelineVisual ?? (!playing ? selectedLocation : undefined);
  const previewAsset = previewLocation?.clip.assetId ? project.assets.find((asset) => asset.id === previewLocation.clip.assetId) : selectedAsset;
  ...
  useEffect(() => {
    ...
    const offset = Math.min(previewLocation.clip.duration, Math.max(0, time - previewLocation.clip.start));
    const sourceTime = previewLocation.clip.sourceStart + offset;
    if (media.readyState >= 1 && Math.abs(media.currentTime - sourceTime) > .12) media.currentTime = sourceTime;
    if (playing && media.paused) media.play().catch(() => setStatus("Browser blocked video playback — press Play again"));
    if (!playing && !media.paused) media.pause();
  }, [time, previewLocation, previewAsset?.kind, playing]);
  const togglePlayback = async () => {
    if (playing) { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); setPlaying(false); return; }
    setPlaying(true);
  };
  ...
  const insertAsset = (asset: Asset) => {
    ...
    if (!track) { setStatus(`Unlock or create a ${kind} track before inserting`); return; }
    const duration = asset.duration || (asset.kind === "image" ? 5 : 10);
    const clip: Clip = { id: crypto.randomUUID(), assetId: asset.id, name: asset.name, kind: asset.kind, start: time, duration, sourceStart: 0, properties: {} };
    const next = touchProject(project, { tracks: project.tracks.map((item) => item.id === track.id ? { ...item, clips: [...item.clips.map((existing) => existing.start >= time ? { ...existing, start: existing.start + duration } : existing), clip] } : item) });
    commit(next); setSelectedAssetId(asset.id); setSelectedClipId(clip.id); setStatus(`${asset.name} inserted at the playhead; later clips moved right`);
  };
  const beginClipDrag = (event: ReactPointerEvent, clip: Clip, mode: "move" | "left" | "right") => {
    event.preventDefault(); event.stopPropagation();
    drag.current = { mode, clip, startX: event.clientX, project }; dragProject.current = project;
    setSelectedAssetId(clip.assetId); setSelectedClipId(clip.id);
    const onMove = (moveEvent: PointerEvent) => {
      const active = drag.current; if (!active) return;
      const delta = Math.round(((moveEvent.clientX - active.startX) / pixelsPerSecond) * 10) / 10;
      let next = active.project;
      if (active.mode === "move") { const proposed = active.clip.start + delta; next = moveClip(next, active.clip.id, snapping ? snappedClipStart(active.project, active.clip.id, proposed, time, 8 / pixelsPerSecond) : proposed); }
      if (active.mode === "left") { const start = Math.min(active.clip.start + active.clip.duration - .1, Math.max(0, active.clip.start + delta)); next = trimClip(next, active.clip.id, start, active.clip.duration - (start - active.clip.start)); }
      if (active.mode === "right") next = trimClip(next, active.clip.id, active.clip.start, active.clip.duration + delta);
      dragProject.current = next; setProject(next);
    };
    const onUp = () => {
      window.removeEventListener("pointermove", onMove); window.removeEventListener("pointerup", onUp);
      if (dragProject.current && dragProject.current !== drag.current?.project) { history.current.commit(dragProject.current); setStatus(drag.current?.mode === "move" ? "Clip moved" : "Clip trimmed"); }
      drag.current = undefined; dragProject.current = undefined;
    };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      <aside className="media-panel" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addFiles(event.dataTransfer.files); }}><div className="panel-heading"><h2>Media</h2><button onClick={() => mediaInput.current?.click()} aria-label="Add media">＋</button></div>{project.assets.length ? <ul className="media-list">{project.assets.map((asset) => <li className={asset.id === selectedAssetId ? "selected" : ""} key={asset.id}><button className="asset-card" onClick={() => { setSelectedAssetId(asset.id); setPlaying(false); }}><span className="asset-thumb">{asset.thumbnail ? <img src={asset.thumbnail} alt="" /> : asset.kind === "audio" && asset.waveform ? <span className="mini-wave" aria-hidden="true">{asset.waveform.slice(0,30).map((peak,index) => <i key={index} style={{height:`${Math.max(2,peak*35)}px`}} />)}</span> : "♫"}</span><span><strong>{asset.name}</strong><small>{asset.kind} · {formatBytes(asset.size)}{asset.duration ? ` · ${asset.duration.toFixed(1)}s` : ""}</small>{!mediaUrls[asset.id] && <em>Media missing — relink required</em>}</span></button><button className="asset-insert" aria-label={`Insert ${asset.name} at playhead`} title="Insert at playhead" onClick={() => insertAsset(asset)}>＋</button><button className="asset-delete" aria-label={`Remove ${asset.name}`} onClick={() => removeAsset(asset)}>×</button></li>)}</ul> : <div className="empty-small"><div className="media-icon">▧</div><strong>Drop media here</strong><p>Video, audio, and images are analyzed and stored only in this browser.</p><button onClick={() => mediaInput.current?.click()}>Choose files</button></div>}</aside>
      <section className="stage" aria-label="Video preview">
        <div className="canvas">{previewAsset && previewUrl && previewLocation ? <>
          {previewAsset.kind === "video" && <video key={`${previewUrl}-${previewLocation.clip.id}`} ref={previewMedia as RefObject<HTMLVideoElement>} src={previewUrl} />}
          {previewAsset.kind === "audio" && <div className="audio-preview"><div className="large-wave">{previewAsset.waveform?.map((peak,index) => <i key={index} style={{height:`${Math.max(3,peak*100)}px`}} />)}</div></div>}
          {previewAsset.kind === "image" && <img src={previewUrl} alt={previewAsset.name} />}
        </> : <div className="canvas-mark"><span>V</span><p>{playing ? "Timeline gap" : selectedAsset ? "Move the playhead onto a clip" : "Preview canvas"}</p><small>{project.canvas.width} × {project.canvas.height} · {project.canvas.frameRate} fps</small></div>}</div>
        <div className="transport"><button disabled={!selectedLocation} onClick={() => { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); setTime(selectedLocation?.clip.start ?? 0); setPlaying(false); }} aria-label="Go to start">|◀</button><button onClick={() => { setPlaying(false); setTime((current) => Math.max(0,current-1/project.canvas.frameRate)); }} aria-label="Previous frame">‹</button><button disabled={!project.tracks.some((track) => track.clips.length)} onClick={togglePlayback} aria-label={playing ? "Pause" : "Play"}>{playing ? "❚❚" : "▶"}</button><button onClick={() => { setPlaying(false); setTime((current) => Math.min(totalDuration,current+1/project.canvas.frameRate)); }} aria-label="Next frame">›</button><output>{formatTime(time)}</output><button disabled={!selectedLocation} onClick={() => { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); setTime(selectedLocation ? selectedLocation.clip.start + selectedLocation.clip.duration : totalDuration); setPlaying(false); }} aria-label="Go to end">▶|</button></div>
        {project.tracks.flatMap((track) => track.kind === "audio" ? track.clips.map((clip) => { const url = clip.assetId ? mediaUrls[clip.assetId] : undefined; return url ? <audio className="timeline-audio" key={clip.id} src={url} ref={(node) => { if (node) timelineAudio.current.set(clip.id, node); else timelineAudio.current.delete(clip.id); }} /> : null; }) : [])}
      </section>
      ...
      <section className="timeline" aria-label="Timeline"><div className="timeline-head" style={{gridTemplateColumns:`190px ${timelineWidth}px`}}><div className="timeline-tools"><span>Timeline</span><button onClick={() => setTimelineZoom((zoom) => Math.max(.5, zoom - .25))} aria-label="Zoom out">−</button><output>{Math.round(timelineZoom*100)}%</output><button onClick={() => setTimelineZoom((zoom) => Math.min(4, zoom + .25))} aria-label="Zoom in">＋</button></div><div className="ruler" onClick={(event) => { const rect=event.currentTarget.getBoundingClientRect(); setTime(Math.max(0,(event.clientX-rect.left)/pixelsPerSecond)); }}>{Array.from({length:Math.floor(Math.max(20,totalDuration)/5)+1},(_,index) => <span key={index} style={{left:index*5*pixelsPerSecond}}>{formatTime(index*5).slice(3,8)}</span>)}</div></div>{project.tracks.map((track) => <div className={`track ${track.locked ? "locked" : ""}`} style={{gridTemplateColumns:`190px ${timelineWidth}px`}} key={track.id}><div className="track-label"><strong>{track.name}</strong><div><button aria-pressed={track.kind === "audio" ? track.muted : track.hidden} onClick={() => commit(updateTrack(project, track.id, track.kind === "audio" ? {muted:!track.muted} : {hidden:!track.hidden}))} aria-label={`${track.kind === "audio" ? "Mute" : "Hide"} ${track.name}`}>{track.kind === "audio" ? (track.muted ? "M" : "♫") : (track.hidden ? "○" : "◉")}</button><button aria-pressed={track.locked} onClick={() => commit(updateTrack(project, track.id, {locked:!track.locked}))} aria-label={`Lock ${track.name}`}>{track.locked ? "▣" : "⌑"}</button></div></div><div className="track-lane" onClick={(event) => { if (event.target !== event.currentTarget) return; const rect=event.currentTarget.getBoundingClientRect(); setTime(Math.max(0,(event.clientX-rect.left)/pixelsPerSecond)); }}><div className="playhead" style={{left:`${time*pixelsPerSecond}px`}} />{track.clips.map((clip) => { const asset = project.assets.find((item) => item.id === clip.assetId); return <div role="button" tabIndex={track.locked ? -1 : 0} aria-disabled={track.locked} key={clip.id} className={`timeline-clip ${clip.kind} ${clip.id === selectedClipId ? "selected" : ""}`} style={{left:`${clip.start*pixelsPerSecond}px`,width:`${Math.max(24,clip.duration*pixelsPerSecond)}px`}} onPointerDown={(event) => !track.locked && beginClipDrag(event, clip, "move")} onClick={(event) => { event.stopPropagation(); const rect=event.currentTarget.getBoundingClientRect(); const offset=Math.min(clip.duration,Math.max(0,(event.clientX-rect.left)/pixelsPerSecond)); setSelectedAssetId(clip.assetId); setSelectedClipId(clip.id); setTime(clip.start+offset); setPlaying(false); setStatus(`Previewing ${clip.name} at ${formatTime(clip.start+offset)}`); }} title={`${clip.name}, ${clip.duration.toFixed(1)} seconds. Click to preview; drag to move; drag edges to trim.`}><i className="trim-handle left" aria-label="Trim clip start" onPointerDown={(event) => !track.locked && beginClipDrag(event, clip, "left")} />{asset?.thumbnail && <img src={asset.thumbnail} alt="" />}<span>{clip.name}</span><i className="trim-handle right" aria-label="Trim clip end" onPointerDown={(event) => !track.locked && beginClipDrag(event, clip, "right")} /></div>; })}</div></div>)}</section>
    </main>
