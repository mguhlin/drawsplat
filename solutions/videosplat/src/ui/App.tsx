...
export function App() {
  ...
  useEffect(() => {
    ...
    if (!media || playing || !selectedLocation || selectedAsset?.kind !== "video") return;
    const offset = Math.min(selectedLocation.clip.duration, Math.max(0, time - selectedLocation.clip.start));
    const sourceTime = selectedLocation.clip.sourceStart + offset;
    if (media.readyState >= 1 && Math.abs(media.currentTime - sourceTime) > .03) media.currentTime = sourceTime;
  }, [time, selectedLocation, selectedAsset?.kind, playing]);
  const togglePlayback = async () => {
    if (playing) { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); setPlaying(false); return; }
    if (selectedAsset?.kind === "video" && previewMedia.current) await previewMedia.current.play();
    setPlaying(true);
  };
  ...
  const beginClipDrag = (event: ReactPointerEvent, clip: Clip, mode: "move" | "left" | "right") => {
    ...
    const onMove = (moveEvent: PointerEvent) => {
      ...
      if (active.mode === "move") next = moveClip(next, active.clip.id, active.clip.start + delta);
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <section className="stage" aria-label="Video preview"><div className="canvas">{selectedAsset && selectedUrl ? <>{selectedAsset.kind === "video" && <video key={selectedUrl} ref={previewMedia as RefObject<HTMLVideoElement>} src={selectedUrl} onLoadedMetadata={(event) => { if (selectedLocation) event.currentTarget.currentTime = selectedLocation.clip.sourceStart; }} onTimeUpdate={(event) => setTime(selectedLocation ? selectedLocation.clip.start + Math.max(0,event.currentTarget.currentTime-selectedLocation.clip.sourceStart) : event.currentTarget.currentTime)} onEnded={() => setPlaying(false)} />}{selectedAsset.kind === "audio" && <div className="audio-preview"><div className="large-wave">{selectedAsset.waveform?.map((peak,index) => <i key={index} style={{height:`${Math.max(3,peak*100)}px`}} />)}</div></div>}{selectedAsset.kind === "image" && <img src={selectedUrl} alt={selectedAsset.name} />}</> : <div className="canvas-mark"><span>V</span><p>{selectedAsset ? "Media needs to be relinked" : "Preview canvas"}</p><small>{project.canvas.width} × {project.canvas.height} · {project.canvas.frameRate} fps</small></div>}</div><div className="transport"><button disabled={!selectedLocation} onClick={() => { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); if (previewMedia.current) previewMedia.current.currentTime = selectedLocation?.clip.sourceStart ?? 0; setTime(selectedLocation?.clip.start ?? 0); setPlaying(false); }} aria-label="Go to start">|◀</button><button disabled={!project.tracks.some((track) => track.clips.length)} onClick={togglePlayback} aria-label={playing ? "Pause" : "Play"}>{playing ? "❚❚" : "▶"}</button><output>{formatTime(time)}</output><button disabled={!selectedLocation} onClick={() => { previewMedia.current?.pause(); timelineAudio.current.forEach((media) => media.pause()); if (previewMedia.current) previewMedia.current.currentTime = selectedLocation ? selectedLocation.clip.sourceStart+selectedLocation.clip.duration : previewMedia.current.duration; setTime(selectedLocation ? selectedLocation.clip.start + selectedLocation.clip.duration : totalDuration); setPlaying(false); }} aria-label="Go to end">▶|</button></div>{project.tracks.flatMap((track) => track.kind === "audio" ? track.clips.map((clip) => { const url = clip.assetId ? mediaUrls[clip.assetId] : undefined; return url ? <audio className="timeline-audio" key={clip.id} src={url} ref={(node) => { if (node) timelineAudio.current.set(clip.id, node); else timelineAudio.current.delete(clip.id); }} onTimeUpdate={(event) => { if (playing && selectedClipId === clip.id) setTime(clip.start + Math.max(0, event.currentTarget.currentTime - clip.sourceStart)); }} onEnded={() => { if (selectedClipId === clip.id) setPlaying(false); }} /> : null; }) : [])}</section>
