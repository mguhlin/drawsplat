...
const defaults = key => ({ key, color: '#00ff00', tolerance: 45, smoothing: 25, spill: false, x: 0, y: 0, scale: 100, rotation: 0, flip: false, left: 0, right: 0, top: 0, bottom: 0, strokes: [] });
...
export function openGreenScreen({ initialSource, insert, download }) {
  ...
  dialog.innerHTML = `
    ...
    <div class="gs-workspace">
      <section class="gs-view" aria-label="Composition preview">
        <canvas id="gsPreview" width="960" height="540" aria-label="Combined scene"></canvas>
        <p class="hint">Background → Subject → Overlay. Select a layer to change its source or settings. All processing stays on your device. Save or add the scene before closing; the studio setup is not saved.</p>
      ...
      <section class="gs-controls" aria-label="Layer controls">
        ...
        <label><input id="gsSpill" type="checkbox"> Reduce green/blue spill (may affect matching clothing)</label>
  ...
  function freezeCamera() {
    ...
    stream?.getTracks().forEach(track => track.stop()); stream = null; cameraLayer = null; cancelAnimationFrame(frame);
